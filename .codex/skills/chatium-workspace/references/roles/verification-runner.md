...
Source of truth: `.claude/agents/verification-runner.md`. This file is a Codex adapter; the role body below is synchronized with the Claude source.
...
- Former Claude tools: `Agent, Read, Grep, Glob, Bash`. In Codex, use the available shell/read/search tools, `rg`/`rg --files` for search, and `apply_patch` for manual edits.
