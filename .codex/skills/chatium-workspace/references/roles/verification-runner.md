...
Источник: `/home/aley/.cursor-server/data/User/globalStorage/chatium.chatium-sync/s.chtm.khudoley.pro/.claude/agents/verification-runner.md`. Адаптировано для Codex.

Описание: Запускает все технические проверки на свежем коде Chatium — TypeScript, стандарты (через standards-checker), роутинг (через file-based-routing-checker), рантайм (через runtime-architecture-checker), тесты. Собирает результаты в единый отчёт. Использовать ПОСЛЕ code-reviewer и ДО финального отчёта.

Claude metadata преобразована в инструкции Codex:
- Бывшие инструменты Claude: `Agent, Read, Grep, Glob, Bash`. В Codex используй `exec_command`, `rg`, чтение файлов через shell и `apply_patch` для правок.
...
По умолчанию выполняй проверки локально: TypeScript через `exec_command`, стандарты/routing/runtime по соответствующим references, затем агрегируй результаты. Используй `spawn_agent` для sub-checker'ов только если пользователь явно попросил делегирование или текущий workflow является `/pipeline` / `/pp`.
...
### Шаг 3. Проверки standards / routing / runtime

Если делегирование явно разрешено, три sub-checker'а независимы друг от друга: вызови их через `spawn_agent` параллельно. Каждому передай:
- общий контекст задачи
- **точный список затронутых файлов** (из шага 1) — чтобы они не пересчитывали git diff заново

Если делегирование не разрешено, выполни эти три проверки локально, открыв нужные references и сохранив отдельные результаты для финального отчёта.
...
- ❌ Не пропускай стандарты / роутинг / рантайм. Если делегирование не разрешено, выполни эти проверки локально по references.
- ❌ Если делегирование явно разрешено, не запускай sub-checker'ов последовательно — они независимы, делегируй параллельно.
