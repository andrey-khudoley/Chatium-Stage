...
| GET | /api/tests/endpoints-check/integration-gc-order-pl-api | api/tests/endpoints-check/integration-gc-order-pl-api.ts | AnyUser | Проба PL API по заказу: query `gcOrderId` (обязательно), `buyerEmail` (опционально — иначе email из Heap-контракта с этим `gc_order_id`). `probeGcOrderPlApi` (`deal_status=payment_waiting`). Ответ: `{ success, skipped?, gcOrderId, buyerEmailSource, probe, error?, at }`. `gc_order_id=test` — ошибка. |
