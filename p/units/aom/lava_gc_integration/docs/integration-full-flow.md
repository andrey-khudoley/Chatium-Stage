...
1. В Heap сохраняются секреты и идентификаторы: ключи Lava, `lava_product_id` / `lava_offer_id` (технический продукт и оффер-шаблон), секрет webhook Lava, домен и ключи GetCourse (PL API).
