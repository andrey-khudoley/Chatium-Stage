...
Полная техническая документация интеграции GetCourse + Lava и ограничений Chatium разнесена по файлам в **`docs/`** — оглавление: **`docs/README.md`**.
...
- Реализованы: API настроек (list, get, save), Heap-таблица settings, репозиторий, lib (бизнес-логика). В `settings.lib` добавлены ключи и строковые геттеры для интеграции Lava/GetCourse (API-ключи, базовый URL Lava, product/offer id, секрет webhook, домен и ключи GetCourse, service token); дефолт для `lava_base_url` — `https://gate.lava.top`. На админке — мастер Lava: поля API-ключа и базового URL → кнопка загрузки каталога (`POST /api/admin/lava/catalog`, lib `lava-api.client`) → выбор пары продукт/оффер → сохранение в `lava_product_id` и `lava_offer_id`; нормализация URL — `shared/lavaBaseUrl.ts`.
