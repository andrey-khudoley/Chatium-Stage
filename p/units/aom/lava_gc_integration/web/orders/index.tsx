...
import { ORDERS_PAGE_NAME, getPageTitle, getHeaderText } from '../../config/project'
import * as settingsLib from '../../lib/settings.lib'
import { customScrollbarStyles } from '../../styles'

const LOG_PATH = 'web/orders/index'
...
export const ordersPageRoute = app.html('/', async (ctx, req) => {
  await loggerLib.writeServerLog(ctx, {
    severity: 7,
    message: `[${LOG_PATH}] запрос страницы заказов`,
  ...
  try {
    ...
    await loggerLib.writeServerLog(ctx, {
      severity: 4,
      message: `[${LOG_PATH}] редирект на логин (нужен Admin)`,
  ...
  return (
    <html>
      <head>
        <title>{getPageTitle(ORDERS_PAGE_NAME, projectName)}</title>
        <meta name="viewport" content="width=device-width, initial-scale=1" />
        <meta charset="UTF-8" />
      ...
      <body>
        ...
        <OrdersPage
          projectTitle={getHeaderText(ORDERS_PAGE_NAME, projectName)}
          indexUrl={indexUrl}
