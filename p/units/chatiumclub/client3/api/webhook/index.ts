...
import * as paymentReaction from '../../lib/paymentReaction.lib'
import * as webhookRepo from '../../repos/webhookLog.repo'

const LOG_PATH = 'api/webhook'

/**
 * POST /api/webhook?token=<secret> — приём входящего вебхука GC об оплате (App C).
 *
 ...
 * 3. Нормализация события через `normalizePaymentEvent`.
 * 4. `processPaymentEvent` → idempotency → invoke('addCommentToDialog') → запись в WebhookLog.
 *
...
export const webhookRoute = app.post('/', async (ctx, req) => {
  ...
  const event = paymentReaction.normalizePaymentEvent(req.body)
  await loggerLib.writeServerLog(ctx, {
    severity: 6,
    ...
    payload: {
      ...
      hasDealId: !!event.dealId,
      amount: event.amount
    }
  })

  let reaction: paymentReaction.ReactionResult
  try {
    reaction = await paymentReaction.processPaymentEvent(ctx, event)
  } catch (e: unknown) {
    ...
    const log = await webhookRepo.create(ctx, {
      ...
      tokenValid: true,
      reactionOk: false,
      reactionErrorCode: 'CLIENT3_INTERNAL_ERROR',
      status: 'error'
    })
    return {
      success: false,
      ...
      logId: String(log.id),
      error: { code: 'CLIENT3_INTERNAL_ERROR', message: msg }
    }
  }
