...
## Назначение
Приложение **`p/units/chatiumclub/client3`** — каркас сценария **payment-reaction** (входящий webhook оплаты GC → действия в Chatium на стороне gateway). Спецификация: `clients/payment-reaction`; UI-контейнер на базе шаблона.
...
## Интеграции
- **GetCourse webhook / gateway:** основной поток по спецификации C обрабатывается на gateway; это приложение — заготовка под UI/настройки при необходимости.
