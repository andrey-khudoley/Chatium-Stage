...
## Назначение
Приложение **`p/units/chatiumclub/client1`** — каркас сценария **landing-lead** (форма → тонкий клиент → gateway → GC Legacy). Спецификация: `clients/landing-lead`; пока наследует UI шаблона.
...
## Интеграции
- **GetCourse / gateway:** исходящие вызовы по сценарию A; пока не реализованы — каркас шаблона.
