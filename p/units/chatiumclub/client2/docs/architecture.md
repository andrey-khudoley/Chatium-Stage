...
## Назначение
Приложение **`p/units/chatiumclub/client2`** — каркас сценария **quiz-table** (таблица ответов → новый API GC). Спецификация: `clients/quiz-table`; пока наследует UI шаблона.
...
## Интеграции
- **GetCourse / gateway:** отправка по новому API по сценарию B; пока не реализовано — каркас шаблона.
