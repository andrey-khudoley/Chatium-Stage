# Chatium Club · client4 (registration-welcome)

## Назначение

Проект **`p/units/chatiumclub/client4`** — контейнер под сценарий **приветствие при регистрации** (`clients/registration-welcome`). Обработка webhook/polling — на gateway; каркас из `template_project`, отдельные Heap-таблицы.
