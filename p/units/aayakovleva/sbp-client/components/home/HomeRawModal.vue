  ...
  <div class="raw-modal-backdrop" @click.self="$emit('close')">
    <div class="raw-modal" role="dialog" aria-modal="true">
      <header class="raw-modal-head">
        ...
        <h2>
          {{ modal.kind === 'webhook' ? 'Webhook payload' : 'Ответ gateway' }}
...
<script>
// Модальное окно «сырое тело» (ответ gateway или входящий webhook). Состояние —
// в orchestrator'е (rawModal); закрытие/копирование — через emit. CSS глобальный.
import { rawJsonString } from '../../shared/sbpHomeFormat'
