...
export const testsPageRoute = app.html('/', async (ctx, req) => {
  ...
  // Анонимный отсеивается выше (requireRealUser), здесь — авторизованный без роли Admin.
  try {
    requireAccountRole(ctx, 'Admin')
    await loggerLib.writeServerLog(ctx, {
      severity: 6,
      message: `[${LOG_PATH}] requireAccountRole Admin пройдена`,
      payload: { displayName: user.displayName }
    })
  } catch (error: unknown) {
    await loggerLib.writeServerLog(ctx, {
  ...
  const panelUrl = isAdmin ? getFullUrl(ROUTES.panel) : ''
  const loginUrl = getFullUrl(ROUTES.login)
  await loggerLib.writeServerLog(ctx, {
    severity: 6,
    message: `[${LOG_PATH}] Переменные после user`,
    payload: { isAdmin, adminUrl, panelUrl, loginUrl, displayName: user.displayName }
  })
