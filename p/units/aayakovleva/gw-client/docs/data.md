...
- `lib/gateway/lavatopDealIntent.ts` — `handleLavatopDealIntent(ctx, params)`: оркестратор Lava.Top deal-потока. Параметры: `offerId`, `productId` (из настроек), `gcDeal` (результат `resolveGcDeal`), `currency`. Порядок: кэш-lookup идемпотентности → фильтр суммы (hard-limit + lavatopMin/Max по рублёвому cost) → `convertRubTo` → проверка MIN\_AMOUNT по валюте → `runWithExclusiveLock('lavatop-offer:'+offerId)` → повторный кэш-lookup → `updateOfferPrice` → `createInvoice` → **`recordRequestLog`** (с 2026-06-01: успешный `createInvoice` записывается в `request_log` с полями `orderNumber=correlationId=dealId` и `amount` в `argsRedacted` — ранее Lava.Top-ветка этого не делала; запись является источником кэша идемпотентности). Вызывается после серверной проверки `areAllOffersAllowed` в `intent-by-deal`.
