...
- `widget_offer_list_type` — режим фильтрации офферов: `'whitelist'` (только указанные) или `'blacklist'` (все, кроме указанных). Парсится `parseOfferListType` из `shared/widgetSettingsTypes.ts`.
- `widget_offer_ids` — comma-separated список `offerId` для whitelist/blacklist. Парсится `parseOfferIds` → `string[]`.
