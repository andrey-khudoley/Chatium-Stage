...
export const templateIntegrationTestsRoute = app.get('/', async (ctx) => {
  ...
  try {
    ...
    push(
      ...
      'settings.lib: getSettingString(PROJECT_NAME)',
      typeof name === 'string'
    )
  } catch (e) {
    push(
  ...
  try {
    ...
    push(
      'settings_get_log_level',
      'settings.lib: getLogLevel',
      false,
      (e as Error)?.message ?? String(e)
    )
  }
  ...
  try {
    ...
    push(
      'settings_repo_findByKey',
      'settings.repo: findByKey',
      false,
      (e as Error)?.message ?? String(e)
    )
  }
  ...
  try {
    ...
    push(
      'dashboard_get_counts',
      'dashboard.lib: getDashboardCounts',
      false,
      (e as Error)?.message ?? String(e)
    )
  }
  ...
  try {
    ...
    push(
      'logger_admin_socket',
      'logger.lib: getAdminLogsSocketId(ctx)',
      typeof socketId === 'string' && socketId.length > 0
    )
  } catch (e) {
    push(
      'logger_admin_socket',
      'logger.lib: getAdminLogsSocketId',
      false,
      (e as Error)?.message ?? String(e)
    )
  }
