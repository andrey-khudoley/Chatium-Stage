...
export const apiSubscriptionPlansByChatRoute = app.get(
  '/by-chat/:feedId/plans',
  async (ctx, req) => {
...
export const apiSubscriptionPlansCreateRoute = app.post('/plans/create', async (ctx, req) => {
  ...
  if (body.durationType === 'months' && body.calendarPeriod === 'specific') {
    if (
      !body.specificPeriodStart ||
      body.specificPeriodStart < 1 ||
      body.specificPeriodStart > 12
    ) {
  ...
  const maxOrder =
    existingPlans.length > 0 ? Math.max(...existingPlans.map((p) => p.sortOrder || 0)) : 0
...
export const apiSubscriptionPlansUpdateRoute = app.post('/plans/:id/update', async (ctx, req) => {
  ...
  if (body.specificPeriodStart !== undefined)
    updateData.specificPeriodStart = body.specificPeriodStart
...
export const apiSubscriptionPlansAddChatRoute = app.post(
  '/plans/:id/add-chat',
  async (ctx, req) => {
    requireRealUser(ctx)
    requireAccountRole(ctx, 'Admin')
    ...
    const maxOrder =
      existingLinks.length > 0 ? Math.max(...existingLinks.map((l) => l.sortOrder || 0)) : 0
...
export const apiSubscriptionPlansRemoveChatRoute = app.post(
  '/plans/:id/remove-chat',
  async (ctx, req) => {
    requireRealUser(ctx)
    requireAccountRole(ctx, 'Admin')
