...
import { findFeedMessageById, findFeedParticipants, getFeedById } from '@app/feed'
...
// Открепить сообщение
export const apiPinnedRemoveRoute = app.post('/:feedId/remove', async (ctx, req) => {
  requireRealUser(ctx)
...
export const apiPinnedGetRoute = app.get('/:feedId', async (ctx, req) => {
  requireRealUser(ctx)
