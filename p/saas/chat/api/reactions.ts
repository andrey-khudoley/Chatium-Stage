...
function normalizeReactions(message: any): MessageReactions {
  ...
  if (
    ...
    typeof message.data.reactions === 'object' &&
    Object.keys(message.data.reactions).length > 0
  ) {
...
export const apiReactionsToggleRoute = app
  ...
  .post('/:feedId/toggle', async (ctx, req) => {
    // Lazy import серверных модулей
    const { findFeedParticipants, findFeedMessageById, updateFeedMessage } = await import(
      '@app/feed'
    )
    ...
    const updatedMessage = await updateFeedMessage(ctx, req.params.feedId, messageId, updateData)
...
export const apiReactionsGetRoute = app.get('/:feedId/:messageId', async (ctx, req) => {
  // Lazy import серверных модулей
...
export const apiReactionsDetailsRoute = app.get('/:feedId/:messageId/details', async (ctx, req) => {
  // Lazy import серверных модулей
  const { findFeedParticipants, findFeedMessageById } = await import('@app/feed')
