...
- `pages/` — Vue‑страницы: **`PanelHomePage.vue`** (главная панель; вкладки Обзор/Вызовы/Доступы; KPI, таблица завершённых `/v1/{op}`, фильтр по дате, raw-модалка), `InviteAcceptPage.vue` (приём инвайта). Старый `HomePage.vue` (лендинг) не используется.
- `components/` — переиспользуемые Vue‑компоненты (Header, AppFooter, GlobalGlitch, LogoutModal).
- `api/` — API‑эндпоинты. File-based: один файл — один эндпоинт с `/`. Включает: **`api/v1/*.ts`** (публичный gateway без сессии Chatium), **`api/gateway-analytics/`** (аналитика вызовов `/v1/{op}` и filter-save, доступ `guardInternalApi`), **`api/access/`** (управление доступами к панели: generate-invite, consume-invite, revoke-invite, revoke-grant, invites, grants).
- `tables/` — Heap‑таблицы (схемы: settings, logs, **panelAccess**, **panelInvites**).
- `repos/` — репозитории (работа с БД: settings, logs, **panelAccess**, **panelInvites**).
...
- `shared/` — общий код (preloader, logLevel, logger syslog RFC 5424, browserRemoteLogger, **`accessPages.tsx`** — JSX-страницы сообщений доступа).
...
Глобальный фильтр по дате/времени хранится в Heap-настройке `panel_date_filter` как объект `{ from?, to? }` (Unix ms). Управляется через `api/gateway-analytics/filter-save.ts` (`guardInternalApi`). Эндпоинт `api/gateway-analytics/invocations.ts` применяет фильтр in-memory (`filters.dateFromMs`/`filters.dateToMs`). SSR-проп `initialDateFilter` передаётся из `index.tsx` в `PanelHomePage.vue`.
