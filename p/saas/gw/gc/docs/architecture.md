...
## Роутинг
- `index.tsx` — панель (`PanelHomePage.vue`; `requireRealUser` + `requireInternalAccess`). Аноним → `/s/auth/signin`; без доступа → `/web/forbidden`. Передаёт `isAdmin`, `initialDateFilter`, расширенные `apiUrls`.
...
- `pages/` — Vue‑страницы: **`PanelHomePage.vue`** (главная панель; вкладки Обзор/Входящие/К GetCourse/Доступы/**Создать запрос**; KPI, таблица завершённых `/v1/{op}`, фильтр по дате, raw-модалка), `InviteAcceptPage.vue` (приём инвайта). Старый `HomePage.vue` (лендинг) не используется.
...
## Вкладка «Создать запрос» (PanelHomePage)

Вкладка с id `request-builder` в `PanelHomePage.vue`; реализована компонентом **`components/RequestTestTab.vue`** (Composition API). Видна при `requireInternalAccess` и непустом каталоге операций (`operationsCatalog` из `lib/gateway/operationsCatalog.ts`).
...
3. Секция «Тело запроса/Параметры» — динамические поля по `argsSchema.fields`; обязательные помечены `*`, опциональные — `(опционально)`; типы string/number/boolean/array/object приводятся при отправке. Кнопка «Подставить» email подставляет `tester@khudoley.pro`; кнопка «Очистить» сбрасывает форму.
...
Глобальный фильтр по дате/времени хранится в Heap-настройке `panel_date_filter` как объект `{ from?, to? }` (Unix ms). Управляется через `api/admin/analytics/filter-save.ts` (`guardInternalApi`). SSR-проп `initialDateFilter` передаётся из `index.tsx` в `PanelHomePage.vue`; панель использует его для `apiUrls.filterSave`.
