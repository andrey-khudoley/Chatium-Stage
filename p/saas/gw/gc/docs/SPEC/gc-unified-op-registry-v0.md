...
При расхождении между локальным OpenAPI и ответом продакшена GC при реализации gateway приоритет у **фактического поведения GC**; затем обновляют JSON в `docs/SPEC/` (этот каталог) и при необходимости таблицы в этом файле. **Имена `op`** меняются только явным решением (новое имя или версия).
...
2. Подмножество `op` под **четыре опорных сценария эфира** — вести от продуктовых материалов курса (демо до эфира, спецификация после эфира) вне этого репозитория и зафиксировать минимальные `args` при первой продуктовой итерации; при желании вынести в отдельный `op-args-webinar-subset.md` в `docs/SPEC/`.
