 ...
 *
 * Пауза без таймеров: в изоляте Chatium глобальный `setTimeout` может отсутствовать.
 * `@app/jobs` (`scheduleJobAfter`, см. inner/docs/005-jobs.md) планирует код на потом и
 * не подменяет блокирующую задержку в рамках одного HTTP-запроса — для короткого
 * троттлинга здесь синхронное ожидание по `Date.now()`.
 */
...
function collectDependencyChain(targetOp: string): V1OpScenario[] {
...
async function runScenariosSequentially(
  ctx: app.Ctx,
...
function makeFreshRunContext(schoolHost: string, schoolApiKey: string): V1OpsRunContext {
  return {
    schoolHost,
    schoolApiKey,
