...
import { interpretLegacyHttpResponse } from './interpretGcV1Response'
import { invokeLegacyGcImportPost } from './legacyGcImportClient'
import { GW_MAX_REQUEST_BODY_BYTES } from './constants'
import { addUserArgsSchema, getAddUserCatalogMeta } from './operationsCatalog'
...
/** Минимальные поля входящего HTTP-запроса (роут платформы или синтетический объект в тестах). */
export type V1AddUserIncomingLike = {
  method?: string
  headers?: unknown
  body?: unknown
}
...
/**
 * Обработчик POST /v1/addUser: валидация, Heap `gc_developer_api_key`, исходящий Legacy-вызов GC.
 * Вынесен в lib для интеграционных тестов с явным `req` (заголовки школы + JSON args).
 */
export async function handleV1AddUserPost(ctx: app.Ctx, req: V1AddUserIncomingLike): Promise<V1TuneResponse> {
  const started = Date.now()
  const requestId = newGatewayRequestId()
  const incomingMethod = String(req.method ?? 'POST').toUpperCase()
