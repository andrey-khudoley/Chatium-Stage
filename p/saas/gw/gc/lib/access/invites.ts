/**
 * Управление пригласительными ссылками и грантами доступа к панели GetCourse Gateway
 * (внутренняя система доступов).
 *
 ...
 * Приоритет: used > revoked > expired: уже использованный инвайт остаётся
 * «использованным» даже если параллельно истёк или отозван.
 */
...
/**
 * Сгенерировать случайный токен ≥ 32 hex-символа.
 * Math.random — платформа Chatium; не требует системных зависимостей.
 */
 ...
 * вызвать `requireAccountRole(ctx, 'Admin')`).
 */
export async function generateInvite(
  ...
  // поэтому нужен полный адрес с host текущего воркспэйса.
 ...
 * Любая ветка отказа инвайт **не трогает**.
 */
