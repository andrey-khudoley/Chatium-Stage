...
export type GcHomeDateTimeParts = { date: string; time: string }

/** Разбивает Unix ms на компоненты date (YYYY-MM-DD) и time (HH:MM) в локальной зоне. */
export function msToDateTimeParts(ms: number | null | undefined): GcHomeDateTimeParts {
...
export function rowClassUpstream(u: { semanticRule?: string; upstreamKind: string }): string {
...
export function defaultGcHomeCounts(): GcHomeCounts {
  return {
    totalRequests: 0,
    totalOk: 0,
    totalErrors: 0,
...
export function defaultGcHomeApiUrls(): GcHomeApiUrls {
  return {
    recentRequests: '',
    recentUpstream: '',
    rawRequest: '',
