...
function getApiBaseUrl(): string {
  ...
  const basePath = path.replace(/\/$/, '') || `/${props.projectRoot}`
      ...
      <div class="tp" :class="{ ready: bootLoaderDone }">
        <div class="tp-toolbar">
          <div class="tp-toolbar-left">
            ...
            <div class="tp-tabs">
              ...
              <button
                v-if="props.isAdmin && props.operationsCatalog && props.operationsCatalog.length"
                type="button"
                class="tp-tab"
                :class="{ active: testsSuiteTab === 'request-test' }"
                @click="testsSuiteTab = 'request-test'"
              >
        ...
        <div class="tp-grid" :class="{ 'tp-grid--logs': props.encodedLogsSocketId }">
          <div class="tp-main content-wrapper">
            <div v-if="!isRequestTestTab" class="tp-metrics">
              <div class="tp-metric">
            ...
            <div
              v-if="props.isAdmin && props.operationsCatalog"
              v-show="testsSuiteTab === 'request-test'"
              class="tp-tab-panel"
            >
              <div class="tp-suite">
                <div class="tp-suite-hd">
                  <h2><i class="fas fa-paper-plane tp-icon-hd"></i> Тест запроса</h2>
                  <code class="tp-code">fetch /{{ props.projectRoot }}/api/v1/&#123;op&#125;</code>
                </div>
                <RequestTestTab
                  :operationsCatalog="props.operationsCatalog"
