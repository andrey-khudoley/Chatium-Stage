<script>
// Модальное окно «сырое тело» (запрос клиента или ответ LifePay). Состояние —
// в HomePage (rawModal); закрытие/копирование — через emit. CSS глобальный (.raw-modal*).
import { rawJsonString } from '../../shared/lifepayHomeFormat'
  ...
  <div class="raw-modal-backdrop" @click.self="$emit('close')">
    <div class="raw-modal" role="dialog" aria-modal="true">
      <header class="raw-modal-head">
        ...
        <h2>
          {{ modal.kind === 'upstream' ? 'Ответ LifePay' : 'Запрос клиента' }}
