...
/**
 * Стили вкладки «Создать запрос» (`RequestTestTab.vue` и под-компоненты `Rt*`).
 * Вынесены из `<style scoped>` SFC ради компактности (001-standards §«KEEP FILES SMALL»):
 * один SFC раскладывается на несколько под-компонентов, а общие `.rt-*` классы инъектируются
 * глобально на каждой странице, где монтируется вкладка (панель + страница тестов).
 */
