      ...
      <div class="content-inner" :class="{ ready: bootLoaderDone }">
            ...
            <section class="kpi-grid kpi-grid-secondary" aria-label="Метрики вебхуков">
              ...
              <article class="stat-card">
                <div class="stat-label">
                  <i class="fas fa-triangle-exclamation"></i> Ошибок форварда
                </div>
        ...
        <section v-show="activeTab === 'webhooks'" class="panel-section">
            ...
            <table class="data-table">
              ...
              <tbody>
                <tr v-for="w in webhooks" :key="w.id" :class="rowClassWebhook(w)">
                  ...
                  <td>
                    <button class="btn-mini" @click="doReforward(w.id)" title="Переслать повторно">
          ...
          <div v-else class="empty-state">
            ...
            <p class="empty-hint">
              Сюда попадут вебхуки Lava.Top (payment.*, subscription.*) и статус их форварда
              клиенту.
            </p>
          </div>
        </section>
    ...
    <div v-if="rawModal" class="raw-modal-backdrop" @click.self="closeRaw">
      <div class="raw-modal" role="dialog" aria-modal="true">
        <header class="raw-modal-head">
          <span class="prompt">›</span>
          <h2>
            {{ rawModal.kind === 'upstream' ? 'Ответ Lava.Top' : 'Запрос клиента' }}
...
export default {
  ...
  props: {
    ...
    projectRoot: { type: String, default: 'p/saas/gw/lavatop' },
    initialDateFilter: {
      type: Object,
      default: () => ({})
    },
  ...
  methods: {
    ...
    async loadWebhooks() {
      const base = this.apiUrls && this.apiUrls.recentWebhooks
      if (!base) return
      try {
        const res = await fetch(this.withDateRange(base), {
          headers: { Accept: 'application/json' }
        ...
        const data = await res.json()
        if (data && data.success && Array.isArray(data.entries)) {
          this.webhooks = data.entries
          this.lastUpdated.webhooks = Date.now()
        }
      } catch (e) {
        log.error('loadWebhooks failed', { error: String(e) })
      }
    },
    async doReforward(id) {
      const url = this.apiUrls && this.apiUrls.reforwardWebhook
      if (!url || !id) return
      try {
        await fetch(url, {
          method: 'POST',
        ...
        log.error('doReforward failed', { error: String(e) })
      }
    },
    ...
    rowClassWebhook(w) {
      return this.isWebhookOk(w) ? 'row-ok' : 'row-err'
    },
    copyText(text) {
      try {
