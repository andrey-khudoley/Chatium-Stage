// @shared
// Чистая (без Vue) логика вкладки «Создать запрос»: легенда/пример формата из дерева `argsTree`,
// сборка/валидация плоского тела по `argsSchema.fields`, форматирование снапшотов и fetch к
// `/v1/{op}`. Вынесено из SFC ради компактности (001-standards §«KEEP FILES SMALL»); функции
// работают с plain-данными. У Lava.Top форма плоская (итерация по `ArgsFieldSchema[]`),
// в отличие от рекурсивного дерева gc — легенда/пример используют полное `argsTree` так же,
// как в gc, а поля ввода строятся по плоскому `argsSchema.fields`.

import type { ArgsTreeNode, ArgsTreeField, ArgsFieldSchema } from './operationsCatalogShared'
...
// ── Плоская форма по полям (argsSchema.fields) ────────────────────────────────
...
export function validateApikey(value: string): string {
  if (!value.trim()) return 'Заголовок X-Lava-Apikey обязателен.'
  return ''
}
...
/** Карта ошибок: обязательный заголовок X-Lava-Apikey + per-field валидация плоских полей. */
export function buildFieldErrors(
  ...
  errs['X-Lava-Apikey'] = validateApikey(apikey)
