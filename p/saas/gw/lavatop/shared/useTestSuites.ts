 ...
 * Вынесено из pages/TestsPage.vue ради лимита размера файла; поведение без изменений.
 *
 * Базовый URL API и проверки HTTP-страниц — лавтоп-специфичные (по projectRoot и
 * SSR-маркеру `saas-gw-lavatop-page`), поэтому заданы локально, а не из testSuiteHelpers.
 */
...
import { HTTP_PATH_BY_TEST_ID, httpPagePassed, getApiBaseUrl } from './lavatopHttpChecks'
...
export function useTestSuites(options: UseTestSuitesOptions) {
  ...
  function apiBaseUrl(): string {
    return getApiBaseUrl(options.indexUrl, options.projectRoot)
  }
