...
Набор: юнит без Heap (`lib/tests/templateUnitSuite.ts`) — в т.ч. **pure-проверки gateway** (crypto на Node `crypto`, opRegistry, …); интеграция с Heap и внутренним `route.run` (`lib/tests/integrationSuite.ts`) — в т.ч. **`api/v1/*`**; HTTP GET страниц на клиенте (`TestsPage.vue`, `fetch` по базовому URL проекта). Каталог — `shared/testCatalog.ts`; страница `/web/tests` — три вкладки (Юнит / Интеграция / HTTP): **Юнит** и **Интеграция** запрашиваются с клиента через `fetch(new URL('../api/tests/unit|integration', window.location.href))` (корректный путь от `/web/tests`). Роуты `api/tests/unit` и `api/tests/integration` **не** помечены `@shared-route`: иначе зависимость `templateUnitSuite` → `crypto.lib` оказывается в shared-бандле и функции вроде `encryptUtf8` не являются реальными Node-реализациями (`is not a function`). Для `GET /web/tests` фрагменты SSR — `window.__BOOT__` и подстрока `gc-api-gateway-page` из `<meta name="gc-api-gateway-page" content="web-tests">` в `web/tests/index.tsx`.
