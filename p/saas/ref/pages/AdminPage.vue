...
const telegramTestBotToken = ref('')
const lastSavedTelegramTestBotToken = ref('')
const telegramTestBotTokenError = ref('')
const telegramTestBotTokenLoading = ref(false)
...
const TELEGRAM_TEST_BOT_TOKEN_KEY = 'telegram_test_bot_token'

const loadTelegramTestBotToken = async () => {
  try {
    const res = await getSettingRoute.query({ key: TELEGRAM_TEST_BOT_TOKEN_KEY }).run(ctx)
    const data = res as { success?: boolean; value?: unknown }
    if (data?.success && typeof data.value === 'string') {
      const loaded = data.value
      telegramTestBotToken.value = loaded
      lastSavedTelegramTestBotToken.value = loaded
    } else if (data?.success && (data.value === null || data.value === undefined)) {
      telegramTestBotToken.value = ''
      lastSavedTelegramTestBotToken.value = ''
    }
  } catch (e) {
    log.warning('Не удалось загрузить токен тестового бота', e)
  }
}

watch(telegramTestBotToken, () => {
  if (telegramTestBotTokenDebounceTimer.id) clearTimeout(telegramTestBotTokenDebounceTimer.id)
  telegramTestBotTokenDebounceTimer.id = setTimeout(() => {
    telegramTestBotTokenDebounceTimer.id = null
    const trimmed = String(telegramTestBotToken.value ?? '').trim()
    if (trimmed !== lastSavedTelegramTestBotToken.value) {
      saveTelegramTestBotToken()
    }
  }, INPUT_DEBOUNCE_MS)
})

const saveTelegramTestBotToken = async () => {
  telegramTestBotTokenError.value = ''
  telegramTestBotTokenLoading.value = true
  const prev = telegramTestBotToken.value
  try {
    const res = await saveSettingRoute.run(ctx, {
      key: TELEGRAM_TEST_BOT_TOKEN_KEY,
      value: telegramTestBotToken.value.trim()
    })
    const data = res as { success?: boolean; error?: string }
    if (data?.success === false) {
      telegramTestBotTokenError.value = data.error || 'Ошибка сохранения'
      telegramTestBotToken.value = prev
      showSaveStatus(telegramTestBotTokenSaveStatus, telegramTestBotTokenStatusTimeout, 'error')
    } else {
      lastSavedTelegramTestBotToken.value = telegramTestBotToken.value.trim()
      showSaveStatus(telegramTestBotTokenSaveStatus, telegramTestBotTokenStatusTimeout, 'saved')
    }
  } catch (e) {
    telegramTestBotTokenError.value = (e as Error)?.message || 'Ошибка сохранения'
    telegramTestBotToken.value = prev
    showSaveStatus(telegramTestBotTokenSaveStatus, telegramTestBotTokenStatusTimeout, 'error')
  } finally {
    telegramTestBotTokenLoading.value = false
  }
}
...
onBeforeUnmount(() => {
  ...
  if (telegramTestBotTokenDebounceTimer.id) {
    clearTimeout(telegramTestBotTokenDebounceTimer.id)
    telegramTestBotTokenDebounceTimer.id = null
  }
  if (telegramTestBotTokenStatusTimeout.id) {
    clearTimeout(telegramTestBotTokenStatusTimeout.id)
    telegramTestBotTokenStatusTimeout.id = null
  }
})
    ...
    <main class="content-wrapper flex-1 relative z-10 min-h-0 overflow-y-auto">
      <div class="content-inner">
        <section class="admin-section" :class="{ 'admin-visible': bootLoaderDone }">
          ...
          <div class="admin-card">
            <span
              v-if="telegramTestBotTokenSaveStatus"
              class="admin-card-status"
              :class="telegramTestBotTokenSaveStatus === 'saved' ? 'status-saved' : 'status-error'"
            >
              {{ telegramTestBotTokenSaveStatus === 'saved' ? 'Сохранено' : 'Ошибка' }}
            </span>
            <div class="admin-card-header">
              <i class="fas fa-vial admin-card-icon"></i>
            ...
            <div class="settings-form">
              <div class="settings-field">
                ...
                <input
                  ...
                  type="password"
                  class="settings-input"
                  placeholder="123456:TEST:TOKEN или токен от @BotFather"
                  autocomplete="off"
                  :disabled="telegramTestBotTokenLoading"
                />
              </div>
            </div>
