...
- API тестов: каталог `api/tests/` (GET /api/tests/list — список категорий и тестов, AnyUser); категория `api/tests/endpoints-check/` с эндпоинтами для проверки всех слоёв: health, ping, config, settings-lib, settings-repo, logger-lib, logs-repo, dashboard-lib. Каждый возвращает массив results по каждой функции. Порядок операций: «создание до чтения» (create/upsert перед read). Страница тестов (`web/tests/index.tsx` + `pages/TestsPage.vue`) содержит шесть блоков — «Проверка эндпоинтов», «Библиотека настроек», «Репозиторий настроек», «Библиотека логов», «Репозиторий логов», «Библиотека админки»; есть дашборд метрик и кнопки запуска всех тестов/групп.
