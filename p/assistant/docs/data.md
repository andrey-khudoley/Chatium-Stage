...
| t__assistant__task_project__9Lp4qR | tables/task-projects.table.ts | Проекты внутри клиента | userId, clientId, name, sortOrder |
| t__assistant__task_item__2Vx8sT | tables/task-items.table.ts | Задачи внутри проекта | userId, projectId, title, description, priority (1–4), status (todo/in_progress/done/cancelled), sortOrder |
...
- `repos/tasks.repo.ts` — getTreeForUser, CRUD клиентов/проектов/задач, reorderTasks (порядок в проекте: два прохода `TaskItems.update(ctx, { id, sortOrder })` — см. Heap, не `(ctx, id, patch)`); удаление клиента каскадом (проекты и задачи), проекта — с задачами.
