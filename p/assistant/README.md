...
## Назначение
Проект `p/assistant` для Chatium. Базовая структура: главная страница (карточки «Мой журнал», «Мои задачи», «Диалоги»), страница «Мой журнал» (`/web/journal` — блокнот и вкладка «День» с задачами «В работе» из Heap, порядок дня и сброс в очередь), страница «Управление задачами» (`/web/tasks` — клиенты → проекты → задачи, приоритеты, статусы, «Отметить» для дня), админка, профиль, логин и тестовая страница.
