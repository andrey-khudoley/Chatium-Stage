...
## Назначение
Проект `p/assistant` для Chatium. Базовая структура: главная страница (карточки «Мой журнал», «Мои задачи», «Диалоги»), страница «Мой журнал» (`/web/journal`), страница «Управление задачами» (`/web/tasks` — клиенты → проекты → задачи в Heap, боковая панель с иерархией и таблица задач в стиле журнала, приоритеты, статусы, порядок), админка, профиль, логин и тестовая страница.
