...
## Текущее состояние
- Главная, админка, профиль и логин существуют как минимальные страницы. На главной ниже hero размещены пять разделов: **Календарь**, **Мой день**, **Неделя**, **Привычки**, **Блокнот** — каждый ведёт на отдельную страницу. Роуты: `web/calendar`, `web/my-day`, `web/week`, `web/habits`, `web/notebook`. Раздел **Блокнот** доступен только авторизованным пользователям (`requireRealUser`): список заметок, просмотр и редактирование по hash-роутингу (#view/:id, #edit/:id, #edit/new); заметки хранятся в Heap (таблица notebook_notes), API: list, get, save, delete, toggleCheckbox; редактор с панелью форматирования (markdown) и чекбоксы с сохранением по клику. На страницах разделов — навигация SectionNav. Адаптивная вёрстка и единый визуальный стиль (CRT-тема, акценты).
...
## Changelog
- 2026-03-14: Блокнот: страница /web/notebook только для авторизованных (requireRealUser). Список заметок, просмотр и редактирование по hash (#view/:id, #edit/:id, #edit/new). Heap-таблица notebook_notes, репозиторий notebookNotes.repo, API api/notebook/ (list, get, save, delete, toggleCheckbox). Редактор с панелью markdown и чекбоксы с сохранением по клику. Документация: architecture, api, data, README, .CHATIUM-LLM.md.
- 2026-03-14: в shared/sectionPageStyles.ts добавлен комментарий // @shared (соответствие стандартам).
- 2026-03-14: UI главной с пятью разделами (Календарь, Мой день, Неделя, Привычки, Блокнот): навигация и страницы-заглушки. В config/routes.tsx добавлены ROUTES и ROUTE_PATHS для calendar, my-day, week, habits, notebook. Пять file-based роутов в web/calendar, web/my-day, web/week, web/habits, web/notebook; пять Vue-страниц (CalendarPage … NotebookPage) с SectionNav и заглушкой «Раздел в разработке». На главной ниже hero — секция с кликабельными карточками разделов; адаптивная вёрстка (320px, 768px, 1200px+). Общие стили разделов в shared/sectionPageStyles.ts. Документация: architecture.md, README, .CHATIUM-LLM.md, docs/imports.md.
