...
function getApiBaseUrl(): string {
  const path = props.indexUrl.startsWith('http')
    ...
    : props.indexUrl
  const basePath = path.replace(/\/$/, '') || '/p/neso/crm'
  const origin =
  ...
  <div class="app-layout bg-[var(--color-bg)] text-[var(--color-text)] flex flex-col">
    ...
    <main class="content-wrapper flex-1 relative z-10 min-h-0 overflow-y-auto">
      <div class="content-inner">
        <section class="tests-section" :class="{ 'content-visible': bootLoaderDone }">
          <div class="tests-header">
            <div class="tests-icon-wrapper">
              <i class="fas fa-flask tests-icon"></i>
            </div>
            <h1 class="tests-heading">Тесты</h1>
            <p class="tests-description">Страница для тестов и проверок</p>
          </div>

          <!-- Логи (тот же сокет, что в админке; только для админа) -->
          <div v-if="bootLoaderDone && props.encodedLogsSocketId" class="tests-logs-card">
          ...
          <div v-if="bootLoaderDone" class="tests-card tests-dashboard">
          ...
          <div v-if="bootLoaderDone" class="tests-card tests-endpoints-card">
...
.tests-icon-wrapper {
  ...
  box-shadow: 0 4px 14px rgba(70, 240, 210, 0.2);
}
