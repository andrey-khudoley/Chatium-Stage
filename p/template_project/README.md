...
- API тестов: каталог api/tests/ (GET /api/tests/list — список категорий и тестов, AnyUser); категория api/tests/endpoints-check/ с отдельными тестами по одному эндпоинту в файле (GET /api/tests/endpoints-check/health, GET /api/tests/endpoints-check/ping). Структура: api/tests/ — общий каталог, api/tests/<категория>/ — директория категории, в каждой по одному файлу на тест с путём `/`.
