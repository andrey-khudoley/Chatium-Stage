...
export const botsPageRoute = app.html('/bots', async (ctx, req) => {
  // Применяем уровень логирования из настроек
  await applyDebugLevel(ctx, 'bots')
  Debug.info(ctx, '[bots] Начало обработки страницы ботов')
  
  try {
    requireRealUser(ctx)
    Debug.info(ctx, `[bots] Пользователь авторизован: userId=${ctx.user.id}`)
  } catch (error: any) {
    Debug.warn(ctx, `[bots] Пользователь не авторизован, перенаправление на страницу входа`)
    Debug.warn(ctx, `[bots] Ошибка авторизации: ${error?.message || 'Unknown error'}`)
    const loginUrl = loginPageRoute.url() + '?back=' + encodeURIComponent(req.url)
    return (
      <html>
        <head>
  ...
  await ensureDefaultSettings(ctx)
  Debug.info(ctx, '[bots] Настройки по умолчанию проверены')
  
  // Получаем projectId из query параметров
  const projectId = req.query.projectId as string | undefined
  
  if (!projectId || !projectId.trim()) {
    Debug.warn(ctx, '[bots] projectId не предоставлен, перенаправление на страницу проектов')
    const projectsUrl = projectsPageRoute.url()
    return (
      <html>
        <head>
  ...
  const trimmedProjectId = projectId.trim()
  Debug.info(ctx, `[bots] Запрос страницы ботов для проекта: projectId=${trimmedProjectId}`)
  
  // Получаем проект и проверяем права доступа
  const project = await Projects.findById(ctx, trimmedProjectId)
  
  if (!project) {
    Debug.warn(ctx, `[bots] Проект с ID ${trimmedProjectId} не найден, перенаправление на страницу проектов`)
    const projectsUrl = projectsPageRoute.url()
    return (
      <html>
        <head>
  ...
  if (!isAdmin) {
    ...
    if (!hasAccess) {
      Debug.warn(ctx, `[bots] Попытка доступа к ботам проекта без прав: userId=${ctx.user.id}, projectId=${trimmedProjectId}`)
      const projectsUrl = projectsPageRoute.url()
      return (
        <html>
          <head>
  ...
  // Получаем название проекта из проекта или из настроек по умолчанию
  const projectTitle = project.name || 'Проект'
  Debug.info(ctx, `[bots] Название проекта: ${projectTitle}`)
  
  // Проверяем авторизацию пользователя
  const isAuthenticated = !!ctx.user
  Debug.info(ctx, `[bots] Статус авторизации: ${isAuthenticated ? `авторизован (userId=${ctx.user?.id})` : 'не авторизован'}`)
  
  Debug.info(ctx, '[bots] Рендеринг страницы ботов')
  return (
    <html>
      <head>
        <title>{projectTitle} - Боты</title>
        <meta name="viewport" content="width=device-width, initial-scale=1" />
        <meta charset="UTF-8" />
        <style>{`
          html { 
