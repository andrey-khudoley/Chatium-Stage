...
export const TAssistantPomodoroState6Gs2mQ = Heap.Table(
  't__assistant__pomodoro_state__6Gs2mQ',
  {
    ...
    phaseRemainingSec: Heap.Optional(
      Heap.Number({ customMeta: { title: 'Остаток в paused/stopped (сек)' } })
    ),
    cyclesCompleted: Heap.Optional(
      Heap.Number({ customMeta: { title: 'Завершено рабочих циклов' } })
    ),
    ...
    cyclesUntilLongRest: Heap.Optional(
      Heap.Number({ customMeta: { title: 'Циклов до длинного отдыха' } })
    ),
    ...
    autoStartNextCycle: Heap.Optional(
      Heap.Boolean({ customMeta: { title: 'Автостарт следующего цикла' } })
    ),
    phaseChangeSound: Heap.Optional(
      Heap.Number({ customMeta: { title: 'Звук смены фазы (1–5)' } })
    ),
    tasksCompletedToday: Heap.Optional(
      Heap.Number({ customMeta: { title: 'Задач завершено сегодня' } })
    ),
    statsPeriodDayKey: Heap.Optional(
      Heap.String({
        customMeta: {
          title: 'Ключ периода дневной статистики (YYYY-MM-DD, полночь по UTC+N из профиля)'
        }
      })
    )
  },
