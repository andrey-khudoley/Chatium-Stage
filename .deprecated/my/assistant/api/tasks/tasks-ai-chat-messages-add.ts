...
export const taskAiChatMessagesAddRoute = app.post(
  '/tasks-ai-chat/:feedId/messages/add',
  async (ctx, req) => {
    ...
    const allSmartUsersIds = [
      ...new Set(
        [added?.author?.id, added?.replyTo?.author?.id, user.id].filter(Boolean) as string[]
      )
    ]
