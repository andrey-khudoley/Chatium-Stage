...
- `web/tasks/index.tsx` — страница «Управление задачами» (`TasksPage.vue`): боковая панель с иерархией «клиент → проекты» (широкая колонка; вверху — вторичная по акценту кнопка «Новый клиент», новый проект — иконкой в строке клиента; название клиента не переключает контекст по клику, красная подсветка строки клиента — если выбран проект этого клиента; порядок клиентов и проектов в сайдбаре — drag-and-drop за рукоять, сохранение `sortOrder` через POST `/api/tasks/clients/reorder` и `/api/tasks/projects/reorder`) и основная область с таблицей задач в визуальном стиле журнала; на узких экранах (до 640px) таблица превращается в карточки с подписями полей (`data-label`); **оговорка по DnD:** поведение нативного drag-and-drop в браузере/встроенном клиенте доведено не до конца (иногда индикатор и фактический дроп расходятся или нужен повтор жеста), но смена порядка доступна и сценарий не блокируется; SSR передаёт дерево задач и URL API (`api/tasks/`) для авторизованного пользователя; без входа — подсказка. Модальные формы создания/редактирования, удаление с подтверждением, смена приоритета/статуса/проекта, кнопки «вверх/вниз» и POST reorder для порядка задач в проекте; кнопка «Отметить» переключает статус: не «В работе» → «В работе» (для дневного списка в журнале), повторный клик — обратно в «К выполнению»; при «В работе» кнопка подсвечена красным; поддержка `?client=` и `?project=` в URL для перехода на клиента/проект. При переименовании проекта в POST `api/tasks/projects/update` передаются только `id` и `name` (без `clientId`), чтобы проект не привязывался к клиенту, выбранному в сайдбаре под другим проектом.
