...
**START.** На уровнях с «+метрики» (pp5+) выполни **обе** команды одной связкой (на pp1–4 не нужно):
...
**Фаза G — Отчёт.** По глубине из матрицы. На уровнях с «+метрики» (pp5+) выполни **обе** команды — это обязательная пара, нельзя вставить «## Метрики цикла» без «## Лимиты подписки»:
...
`--track-end` гасит таймер и печатает «## Лимиты подписки»: остаток окон (5ч/7дн) + колонки Δ расхода за прогон и start → end (с учётом сброса). **Фоллбэк:** если `--track-start` на START был пропущен (нет start-записи) — всё равно покажи остаток, запустив `node scripts/usage-limits.mjs` (режим по умолчанию, без Δ). Блок «## Лимиты подписки» в финальном отчёте на pp5+ — обязателен, его отсутствие = недоделанный отчёт. Скрипт сам резолвит OAuth-токен и при его отсутствии/ошибке печатает короткую пометку (отчёт не ломает). Формат отчёта — ниже.
...
<блок «## Метрики цикла» — обязателен на pp5+ (токены по моделям)>
<блок «## Лимиты подписки» — ОБЯЗАТЕЛЕН на pp5+, сразу после метрик: остаток окон (сколько лимита осталось) + Δ за прогон (сколько ушло)>
```
