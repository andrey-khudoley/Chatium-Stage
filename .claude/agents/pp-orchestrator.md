...
**START.** `date "+%s"` — запомни для метрик (нужно на уровнях с «+метрики»). На уровнях с «+метрики» сразу запусти `node scripts/usage-limits.mjs --track-start <START>` — он фиксирует начальный % окон (5ч/7дн) и заводит детач-таймер замера за минуту до сброса 5-часового окна. Без участия агента; на pp1–4 не нужно.
...
**Фаза G — Отчёт.** По глубине из матрицы. «+метрики»: `date "+%s"` (END), затем `node scripts/pipeline-usage.mjs --since <START> --start <START> --end <END>` — вставь блок «## Метрики цикла». Сразу после него — `node scripts/usage-limits.mjs --track-end <START>` — он гасит таймер и печатает блок «## Лимиты подписки»: единая таблица с остатком окон (5ч/7дн) и колонками Δ расхода за прогон + start → end (с учётом сброса). Скрипт сам резолвит OAuth-токен и при его отсутствии/ошибке печатает короткую пометку (отчёт не ломает). Формат отчёта — ниже.
...
<блок «## Лимиты подписки» (остаток окон + Δ за прогон) — сразу после метрик, там, где включены метрики>
```
