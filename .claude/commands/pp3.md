...
Профиль: acceptance inline → мини-план основным чатом → `implementer` (Sonnet) → `code-reviewer` (Sonnet, диф-скоуп, покрывает и стандарты/роутинг/рантайм/платформу) + Bash типы/стиль → fix ≤1 → краткий отчёт. Вопросов не задавай — действуй по дефолтам.
