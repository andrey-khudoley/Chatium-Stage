...
<template>
  <nav>
    <a href="../settings">Настройки</a>  <!-- ✅ OK, относительный путь -->
    <a href="./profile">Профиль</a>      <!-- ✅ OK, относительный путь -->
  </nav>
</template>
```
...
```vue
<template>
  <a :href="withProjectRoot(customFieldsPageRoute.url())">
    Дополнительные поля
  </a>
</template>
...
const searchUrl = withProjectRoot(searchRoute.query({ 
  q: 'chatium', 
  page: '1',
  limit: '10' 
}).url())
