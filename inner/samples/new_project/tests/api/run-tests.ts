...
import {
  apiGetAdminSettingsRoute,
  apiUpdateAdminSettingsRoute
 ...
 */
export async function runTest(ctx: RichUgcCtx, category: string, testName: string): Promise<TestResult> {
  try {
    ...
    if (!categoryDef) {
      Debug.error(ctx, `[tests/${category}/${testName}] ОШИБКА: Категория не найдена: ${category}`, 'E_TEST_CATEGORY_NOT_FOUND')
      Debug.throw(ctx, `[tests/${category}/${testName}] Категория не найдена: ${category}`, 'E_TEST_CATEGORY_NOT_FOUND')
    }
    ...
    if (!testDef) {
      Debug.error(ctx, `[tests/${category}/${testName}] ОШИБКА: Тест не найден в категории ${category}: ${testName}`, 'E_TEST_NOT_FOUND')
      Debug.throw(ctx, `[tests/${category}/${testName}] Тест не найден в категории ${category}: ${testName}`, 'E_TEST_NOT_FOUND')
    }
    Debug.info(ctx, `[tests/${category}/${testName}] Тест найден: ${testDef.description}`)
    
    Debug.info(ctx, `[tests/${category}/${testName}] Маршрутизация к обработчику категории: ${category}`)
    ...
    switch (category) {
      ...
      case 'database':
        ...
        break
      default:
        Debug.error(ctx, `[tests/${category}/${testName}] ОШИБКА: Неизвестная категория: ${category}`, 'E_TEST_UNKNOWN_CATEGORY')
        Debug.throw(ctx, `[tests/${category}/${testName}] Неизвестная категория: ${category}`, 'E_TEST_UNKNOWN_CATEGORY')
    }
  ...
  switch (testName) {
    ...
    case 'api_admin_settings_get':
      try {
        ...
        Debug.throw(ctx, `[tests/admin/api_admin_settings_get] Ошибка теста: ${error.message}`, 'E_TEST_ERROR')
      }

    case 'api_admin_settings_update_project_name':
      try {
        Debug.info(ctx, `[tests/admin/api_admin_settings_update_project_name] Тест обновления названия проекта`)
        ...
        if ((result as any).settings.project_name !== testName) {
          return { success: false, message: `Название не обновлено: expected=${testName}, got=${(result as any).settings.project_name}` }
        }
        
        Debug.info(ctx, `[tests/admin/api_admin_settings_update_project_name] Тест завершён успешно`)
        return { success: true, message: 'Название проекта успешно обновлено' }
      } catch (error: any) {
        Debug.throw(ctx, `[tests/admin/api_admin_settings_update_project_name] Ошибка теста: ${error.message}`, 'E_TEST_ERROR')
      }

    case 'api_admin_settings_update_project_description':
      try {
        Debug.info(ctx, `[tests/admin/api_admin_settings_update_project_description] Тест обновления описания проекта`)
        ...
        if ((result as any).settings.project_description !== testDesc) {
          return { success: false, message: `Описание не обновлено: expected=${testDesc}, got=${(result as any).settings.project_description}` }
        }
        
        Debug.info(ctx, `[tests/admin/api_admin_settings_update_project_description] Тест завершён успешно`)
        return { success: true, message: 'Описание проекта успешно обновлено' }
      } catch (error: any) {
        Debug.throw(ctx, `[tests/admin/api_admin_settings_update_project_description] Ошибка теста: ${error.message}`, 'E_TEST_ERROR')
      }

    case 'api_admin_settings_update_log_level':
      try {
        Debug.info(ctx, `[tests/admin/api_admin_settings_update_log_level] Тест обновления уровня логирования`)
        ...
        if ((result as any).settings.log_level !== 'warn') {
          return { success: false, message: `Уровень логирования не обновлён: expected=warn, got=${(result as any).settings.log_level}` }
        }
        ...
        Debug.throw(ctx, `[tests/admin/api_admin_settings_update_log_level] Ошибка теста: ${error.message}`, 'E_TEST_ERROR')
      }

    case 'api_admin_settings_invalid_log_level':
      try {
        Debug.info(ctx, `[tests/admin/api_admin_settings_invalid_log_level] Тест с недопустимым уровнем логирования`)
        ...
        if (!(result as any).error || !(result as any).error.includes('Недопустимый уровень')) {
          return { success: false, message: `Ожидалось сообщение о недопустимом уровне, получено: ${(result as any).error}` }
        }
        ...
        Debug.throw(ctx, `[tests/admin/api_admin_settings_invalid_log_level] Ошибка теста: ${error.message}`, 'E_TEST_ERROR')
      }
    ...
    case 'api_admin_logs_get_all':
      try {
        ...
        return { success: true, message: `Логи успешно получены: ${(result as any).logs.length} записей` }
      } catch (error: any) {
        Debug.throw(ctx, `[tests/admin/api_admin_logs_get_all] Ошибка теста: ${error.message}`, 'E_TEST_ERROR')
      }

    case 'api_admin_logs_get_filtered':
      try {
        Debug.info(ctx, `[tests/admin/api_admin_logs_get_filtered] Тест получения логов с фильтрацией`)
        ...
        if (nonErrorLogs.length > 0) {
          return { success: false, message: `Найдены логи с другим уровнем: ${nonErrorLogs.length}` }
        }
        
        Debug.info(ctx, `[tests/admin/api_admin_logs_get_filtered] Тест завершён успешно`)
        return { success: true, message: `Логи с фильтром успешно получены: ${(result as any).logs.length} записей уровня ERROR` }
      } catch (error: any) {
        Debug.throw(ctx, `[tests/admin/api_admin_logs_get_filtered] Ошибка теста: ${error.message}`, 'E_TEST_ERROR')
      }

    case 'api_admin_logs_get_paginated':
      try {
        Debug.info(ctx, `[tests/admin/api_admin_logs_get_paginated] Тест получения логов с пагинацией`)
        ...
        if ((result as any).logs.length > 10) {
          return { success: false, message: `Возвращено больше логов, чем ожидалось: ${(result as any).logs.length}` }
        }
        
        if ((result as any).limit !== 10 || (result as any).offset !== 0) {
          return { success: false, message: `Неверные параметры пагинации: limit=${(result as any).limit}, offset=${(result as any).offset}` }
        }
        
        Debug.info(ctx, `[tests/admin/api_admin_logs_get_paginated] Тест завершён успешно`)
        return { success: true, message: `Пагинация работает корректно: ${(result as any).logs.length} записей` }
      } catch (error: any) {
        Debug.throw(ctx, `[tests/admin/api_admin_logs_get_paginated] Ошибка теста: ${error.message}`, 'E_TEST_ERROR')
      }
    ...
    case 'api_admin_logs_counts':
      try {
        ...
        if (typeof (result as any).counts.info !== 'number' || 
        ...
        if (!(result as any).accumulatedCounts || typeof (result as any).accumulatedCounts !== 'object') {
          return { success: false, message: 'Накопленные счётчики не возвращены' }
        }
        
        if (typeof (result as any).accumulatedCounts.error !== 'number' || 
        ...
        Debug.throw(ctx, `[tests/admin/api_admin_logs_counts] Ошибка теста: ${error.message}`, 'E_TEST_ERROR')
      }
    ...
    case 'api_admin_logs_counters_increment':
      try {
        ...
        if (!(beforeResult as any).success) {
          return { success: false, message: `Ошибка получения счётчиков: ${(beforeResult as any).error}` }
        }
        ...
        Debug.info(ctx, `[tests/admin/api_admin_logs_counters_increment] Счётчики до теста: ERROR=${beforeErrorCount}, WARN=${beforeWarnCount}`)
        ...
        if (!(afterResult as any).success) {
          return { success: false, message: `Ошибка получения счётчиков после создания логов: ${(afterResult as any).error}` }
        }
        ...
        Debug.info(ctx, `[tests/admin/api_admin_logs_counters_increment] Счётчики после теста: ERROR=${afterErrorCount}, WARN=${afterWarnCount}`)
        
        // Проверяем, что счётчики увеличились
        if (afterErrorCount !== beforeErrorCount + 1) {
          return { success: false, message: `Счётчик ошибок не увеличился: before=${beforeErrorCount}, after=${afterErrorCount}, expected=${beforeErrorCount + 1}` }
        }
        
        if (afterWarnCount !== beforeWarnCount + 1) {
          return { success: false, message: `Счётчик предупреждений не увеличился: before=${beforeWarnCount}, after=${afterWarnCount}, expected=${beforeWarnCount + 1}` }
        }
        
        Debug.info(ctx, `[tests/admin/api_admin_logs_counters_increment] Тест завершён успешно`)
        return { success: true, message: `Счётчики успешно увеличились: ERROR: ${beforeErrorCount} → ${afterErrorCount}, WARN: ${beforeWarnCount} → ${afterWarnCount}` }
      } catch (error: any) {
        Debug.throw(ctx, `[tests/admin/api_admin_logs_counters_increment] Ошибка теста: ${error.message}`, 'E_TEST_ERROR')
      }
    ...
    case 'api_admin_logs_reset_counters':
      try {
        ...
        Debug.throw(ctx, `[tests/admin/api_admin_logs_reset_counters] Ошибка теста: ${error.message}`, 'E_TEST_ERROR')
      }
    ...
    case 'api_admin_logs_socket_id':
      try {
        ...
        if (!(result as any).encodedSocketId || typeof (result as any).encodedSocketId !== 'string') {
        ...
        Debug.throw(ctx, `[tests/admin/api_admin_logs_socket_id] Ошибка теста: ${error.message}`, 'E_TEST_ERROR')
      }
  ...
  switch (testName) {
    ...
    case 'page_profile':
      try {
        ...
        Debug.throw(ctx, `[tests/pages/page_profile] Ошибка теста: ${error.message}`, 'E_TEST_ERROR')
      }
...
async function runDatabaseTest(ctx: RichUgcCtx, testName: string): Promise<TestResult> {
  ...
  switch (testName) {
    case 'table_settings_exists':
      try {
        Debug.info(ctx, `[tests/database/table_settings_exists] Проверка доступности таблицы Settings`)
        ...
        Debug.throw(ctx, `[tests/database/table_settings_exists] Таблица недоступна: ${error.message}`, 'E_TEST_DB_ERROR')
      }

    case 'table_logs_exists':
      try {
        Debug.info(ctx, `[tests/database/table_logs_exists] Проверка доступности таблицы ProjectLogs`)
        ...
        Debug.throw(ctx, `[tests/database/table_logs_exists] Таблица недоступна: ${error.message}`, 'E_TEST_DB_ERROR')
      }

    default:
      Debug.throw(ctx, `[tests/database/${testName}] Неизвестный тест: ${testName}`, 'E_TEST_UNKNOWN')
  }
}
