...
export const THairdresserBookingBookings2tk = Heap.Table(
  't_hairdresserBooking_bookings_2tk_PdMMWj_IkU_IkU',
  {
    ...
    clientPhone: Heap.Optional(
      Heap.String({ customMeta: { title: 'Телефон клиента' }, searchable: { langs: ['ru', 'en'], embeddings: true } }),
    ),
    clientEmail: Heap.Optional(
      Heap.String({ customMeta: { title: 'Email клиента' }, searchable: { langs: ['ru', 'en'], embeddings: true } }),
    ),
    service: Heap.Optional(
      Heap.String({ customMeta: { title: 'Услуга' }, searchable: { langs: ['ru', 'en'], embeddings: true } }),
    ),
    date: Heap.Optional(
      Heap.DateTime({ customMeta: { title: 'Дата записи' }, searchable: { langs: ['ru', 'en'], embeddings: true } }),
    ),
    time: Heap.Optional(
    ...
    notes: Heap.Optional(
      Heap.String({ customMeta: { title: 'Примечания' }, searchable: { langs: ['ru', 'en'], embeddings: true } }),
    ),
  },
