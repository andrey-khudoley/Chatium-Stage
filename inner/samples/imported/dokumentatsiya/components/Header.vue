  ...
  <header class="bg-white border-b border-gray-200 px-4 py-3 pt-6">
    ...
    <div class="flex items-center justify-between pb-3 mb-4 border-gray-100">
      ...
      <div class="flex items-center space-x-3">
        ...
        />
        <kbd class="absolute right-3 top-1/2 transform -translate-y-1/2 px-2 py-1 text-xs font-semibold text-gray-500 bg-gray-100 border border-gray-200 rounded">
          ⌘ K
        </kbd>
        
        <!-- Search Results Dropdown -->
        <div v-if="showResults && searchResults.length > 0" class="absolute top-full left-0 right-0 mt-1 bg-white border border-gray-200 rounded-lg shadow-lg max-h-96 overflow-y-auto z-10">
          <button
          ...
          >
            <div class="flex items-center space-x-2">
              <img
                ...
                :alt="result.title"
              />
              <i v-else-if="result.icon" :class="`fas fa-${result.icon} text-gray-400 text-sm`"></i>
              <i v-else :class="`fas ${result.type === 'section' ? 'fa-folder' : 'fa-file-lines'} text-gray-400 text-sm`"></i>
              <div class="flex-1 min-w-0">
                <div class="font-medium text-gray-900 truncate">{{ result.title }}</div>
                <div class="text-xs text-gray-500">{{ result.type === 'section' ? ctx.t('Section') : ctx.t('Page') }}</div>
              </div>
            </div>
          </button>
        </div>
          ...
          <!-- No Results Message -->
          <div v-else-if="showResults && searchQuery.length >= 2" class="absolute top-full left-0 right-0 mt-1 bg-white border border-gray-200 rounded-lg shadow-lg p-4 text-center text-sm text-gray-500 z-10">
    ...
    <nav class="hidden md:flex items-center justify-between">
      ...
      >
        ...
        <i
          ...
          :class="['fas', `fa-${section.icon}`, 'text-sm']"
        ></i>
        {{ section.title }}
      </a>
      </div>
      ...
      <div v-if="ctx.user?.is('Admin')" class="flex items-center space-x-2">
        ...
        <button
          ...
          :class="[
            ...
            isViewMode ? 'bg-blue-600 text-white hover:bg-blue-700' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
          ]"
        >
          <i :class="isViewMode ? 'fas fa-pen' : 'fas fa-eye'" class="mr-1"></i>{{ isViewMode ? ctx.t('Edit') : ctx.t('View') }}
        </button>
      </div>
    </nav>
    </div>
...
const emit = defineEmits(['section-change', 'edit', 'navigate', 'navigate-home', 'toggle-menu', 'toggle-view-mode', 'add-section', 'add-page'])
...
watch(() => props.activeSectionId, (newValue) => {
