<template>
  <div class="voice-message-wrapper">
    ...
    <div class="voice-message" :class="{ 'is-own': isOwn }">
      ...
      <!-- Кнопка транскрибации - только для админов, если еще нет транскрипции -->
      <button 
        v-if="isWorkspaceAdmin && !displayedTranscription && !isTranscribing && !messageTranscription"
        class="btn-transcribe-icon"
      ...
      <!-- Индикатор загрузки транскрибации -->
      <div v-else-if="isTranscribing" class="transcribing-indicator" title="Расшифровка...">
    ...
    <!-- Результат транскрибации - виден всем под аудио -->
    <div v-if="displayedTranscription" class="transcription-result">
...
function getColors() {
  ...
  const primary = styles.getPropertyValue('--primary-color').trim() || '#2AABEE'

  if (props.isOwn) {
    return {
      primary,
      bg: 'rgba(42, 171, 238, 0.25)',
      played: '#2AABEE',
      playedBg: 'rgba(42, 171, 238, 0.45)'
    }
  } else {
    return {
      primary,
...
/* Кнопка транскрибации (иконка справа) */
.btn-transcribe-icon {
  width: 28px;
  height: 28px;
  border-radius: 50%;
...
.btn-transcribe-icon:hover {
  background: rgba(42, 171, 238, 0.12);
  color: #2AABEE;
}

.btn-transcribe-icon:disabled {
  opacity: 0.5;
  cursor: not-allowed;
}
