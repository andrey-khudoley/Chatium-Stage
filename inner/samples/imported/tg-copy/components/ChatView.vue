<template>
  <div class="chat-view chat-pattern-bg">
    ...
    <header v-else class="chat-header">
      ...
      <div class="chat-header-actions">
        ...
        <template v-if="!isMobile">
          <button v-if="canManage" @click="showSearch = true" class="btn-icon" title="Поиск">
            <i class="fas fa-search"></i>
          </button>
          <button v-if="canManage && !isDirectChat" @click="showInviteModal = true" class="btn-icon" title="Пригласить участника">
            <i class="fas fa-user-plus"></i>
          </button>
          <button v-if="!isDirectChat && (isOwner || isAdmin || !isChannel)" @click="showParticipants = !showParticipants" class="btn-icon" :title="isChannel ? 'Подписчики' : 'Участники'">
            <i class="fas fa-users"></i>
          </button>
        </template>
        <button @click="activeModal = 'extra-menu'" class="btn-icon" title="Меню">
          ...
          >
            ...
            <div class="message-content">
              ...
              <div 
                ...
                @touchstart.passive="!isSelectionMode && isMobile ? handleMessageTouchStart($event, message) : null"
                @touchend="!isSelectionMode && isMobile ? handleMessageTouchEnd($event) : null"
                @touchmove="!isSelectionMode && isMobile ? handleMessageTouchMove($event) : null"
              >
                <!-- Источник пересылки -->
                <ForwardedFrom
    ...
    <div v-else-if="isMember && canPostMessages" ref="inputAreaRef" class="input-area">
      <!-- Inline запись голосового сообщения -->
      <InlineVoiceRecorder
        v-if="showInlineVoiceRecorder"
        @recorded="onInlineVoiceRecorded"
        @cancel="showInlineVoiceRecorder = false"
      />
      ...
      <form @submit.prevent="sendMessage" class="input-form">
        ...
        <AttachMenu
          ...
          @voice-record="showInlineVoiceRecorder = true"
        ...
        <!-- Кнопка отправки/записи голосового (как в Telegram) -->
        <button 
          v-if="newMessage.trim() || selectedFiles.length > 0"
          type="submit" 
        ...
        <button 
          ...
          type="button" 
          class="btn-send"
          @click="showInlineVoiceRecorder = true"
        >
    ...
    <UserProfileModal
      ...
      :current-user-role="ctx.user?.accountRole"
  ...
  <!-- Inline запись голосового сообщения - убрана модалка, теперь inline в поле ввода -->
</template>
...
import { apiParticipantsListRoute, apiParticipantsRemoveRoute, apiParticipantsUpdateRoleRoute } from '../api/participants'
...
import InlineVoiceRecorder from './InlineVoiceRecorder.vue'
...
const showInlineVoiceRecorder = ref(false)
...
function handleMentionInput(event) {
  ...
  if (lastAtIndex !== -1) {
    ...
    // Проверяем, что @ находится в начале строки или после пробела/не-alphanumeric символа
    // Это предотвращает показ пикера при вводе email (например, user@example.com)
    const charBeforeAt = lastAtIndex > 0 ? textBeforeCursor[lastAtIndex - 1] : null
    const isValidAtPosition = !charBeforeAt || /\s/.test(charBeforeAt) || /[^a-zA-Z0-9_]/.test(charBeforeAt)
    
    // Если после @ нет пробела, позиция валидна и мы не в середине слова
    if (!hasSpaceAfterAt && isValidAtPosition && (textAfterCursor.length === 0 || textAfterCursor.startsWith(' ') || cursorPosition === textBeforeCursor.length)) {
...
async function setupChatChannelSubscription() {
  ...
  try {
    ...
    chatChannelSubscription = subscription.listen((data) => {
      console.log('[ChatView] WebSocket data from chat channel:', { 
        type: data?.type,
        event: data?.event,
        channel: channelName,
        feedId: data?.feedId,
        currentFeedId: props.feedId,
        isForThisChat: data?.feedId === props.feedId,
        hasMessage: !!data?.message,
        messageId: data?.message?.id,
        messageType: data?.message?.type,
        isSystemMessage: data?.message?.data?.isSystemMessage,
        systemType: data?.message?.data?.systemType
      })
      
      // Обрабатываем chat-event от этого чата
      if (data?.type === 'chat-event' && data?.feedId === props.feedId) {
        console.log('[ChatView] Processing chat-event from chat channel:', data.event)
        switch (data.event) {
          case 'new-message':
          ...
          case 'new-participant':
            console.log('[ChatView] Processing new-participant from chat channel', { user: data.user })
            handleNewParticipant(data)
            break
          case 'participant-left':
            console.log('[ChatView] Processing participant-left from chat channel', { userId: data.userId })
          ...
          case 'typing-stop':
            ...
            break
          case 'message-pinned':
            console.log('[ChatView] Processing message-pinned event', data.message)
            pinnedMessage.value = data.message
            break
          case 'message-unpinned':
            console.log('[ChatView] Processing message-unpinned event')
...
watch(socketData, (data) => {
  ...
  if (data.type === 'chat-event' && data.feedId === props.feedId) {
    console.log('[ChatView] Processing chat-event:', data.event)
    switch (data.event) {
      case 'new-message':
        handleNewMessage(data.message)
        break
      ...
      case 'delete-message':
        ...
        break
      case 'new-participant':
        console.log('[ChatView] Processing new-participant from user channel', { user: data.user })
        handleNewParticipant(data)
        break
      case 'participant-left':
        console.log('[ChatView] Processing participant-left from user channel', { userId: data.userId })
        handleParticipantLeft(data)
        break
      case 'reaction-toggle':
        handleReactionUpdate(data)
      ...
      case 'typing-stop':
        ...
        break
      case 'message-pinned':
        console.log('[ChatView] Processing message-pinned from user channel', data.message)
        pinnedMessage.value = data.message
        break
      case 'message-unpinned':
        console.log('[ChatView] Processing message-unpinned from user channel')
        pinnedMessage.value = null
        break
      case 'message-read':
        handleMessageRead(data)
...
function handleNewMessage(message) {
  console.log('[ChatView] handleNewMessage called', { 
    ...
    type: message.type,
    isSystem: message.type === 'System' || message.data?.isSystemMessage,
    hasData: !!message.data,
    dataIsSystem: message.data?.isSystemMessage,
    dataSystemType: message.data?.systemType,
    text: message.text?.substring(0, 50),
    loadedMessageIdsSize: loadedMessageIds.value.size,
    hasMessageId: loadedMessageIds.value.has(message.id)
  })
  
  if (!loadedMessageIds.value.has(message.id)) {
    console.log('[ChatView] Adding new message to list:', message.id)
    loadedMessageIds.value.add(message.id)
    const normalizedMessage = normalizeMessageFields(message)
    
    console.log('[ChatView] Message normalized', { 
      messageId: normalizedMessage.id, 
      type: normalizedMessage.type,
      isSystemMessage: normalizedMessage.data?.isSystemMessage 
    })
    ...
    messages.value.push(normalizedMessage)
    console.log('[ChatView] Message added, total messages:', messages.value.length)
    ...
    console.log('[ChatView] Message already in loadedMessageIds, skipping:', message.id)
  }
}
...
function handleParticipantLeft(data) {
  console.log('[ChatView] handleParticipantLeft called', { 
    userId: data.userId,
    currentParticipantsCount: participants.value.length 
  })
  // Удаляем участника из локального списка (приводим к строке для сравнения)
  const leftUserId = String(data.userId)
  const beforeCount = participants.value.length
  participants.value = participants.value.filter(p => String(p.userId) !== leftUserId)
  console.log('[ChatView] Participant removed, count:', participants.value.length, '(was:', beforeCount, ')')
}

async function handleNewParticipant(data) {
  console.log('[ChatView] handleNewParticipant called', { 
    userId: data.user?.id, 
  ...
  const existingParticipant = participants.value.find(p => String(p.userId) === String(data.user?.id))
  if (existingParticipant) {
    console.log('[ChatView] Participant already exists, skipping')
    return
  }
  
  // Добавляем нового участника в локальный список (для мгновенного отображения)
  ...
  console.log('[ChatView] Participant added locally, new count:', participants.value.length)
  
  // Перезагружаем полный список участников с сервера для синхронизации
  try {
    const response = await apiParticipantsListRoute({ feedId: props.feedId }).run(ctx)
    if (response.participants) {
      participants.value = response.participants
      console.log('[ChatView] Participants reloaded from server, count:', participants.value.length)
    }
  } catch (err) {
    console.error('[ChatView] Failed to reload participants:', err)
  }
}

// Нормализация data на клиенте (защита от строки вместо объекта)
...
function normalizeMessageFields(message) {
  // Нормализуем data
  const normalizedData = normalizeClientData(message.data)
  
  let reactions = message.reactions || normalizedData.reactions || {}
  if (typeof reactions === 'string') {
    try {
      reactions = JSON.parse(reactions)
    } catch (e) {
  ...
  let files = message.files || normalizedData.files || []
  ...
  const forwardedFrom = normalizedData.forwardedFrom || null
  
  // Нормализуем тип сообщения (System, Message, etc.)
  // Системные сообщения могут быть помечены через type или через data.isSystemMessage/systemType
  const isSystem = normalizedData.isSystemMessage === true || normalizedData.systemType !== undefined
  const messageType = isSystem ? 'System' : (message.type || 'Message')
  
  // Логируем для отладки системных сообщений
  if (isSystem || message.type === 'System') {
    console.log('[ChatView] normalizeMessageFields - system message detected', {
      messageId: message.id,
      originalType: message.type,
      isSystemMessage: normalizedData.isSystemMessage,
      systemType: normalizedData.systemType,
  ...
  return {
    ...
    data: {
      ...normalizedData,
      forwardedFrom,
      isSystemMessage: isSystem ? true : normalizedData.isSystemMessage,
    },
  }
}
...
// Обработчики longpress для мобильных
...
function handleMessageTouchStart(event, message) {
  ...
  longPressTimer = setTimeout(() => {
    // Долгое нажатие - показываем меню
    longPressTimer = null
    showMobileMessageMenu(message)
...
function handleMessageTouchEnd(event) {
  // Если таймер ещё активен - это был короткий тап
  if (longPressTimer) {
    clearTimeout(longPressTimer)
    longPressTimer = null
    // Короткий тап - не делаем ничего (не вызываем меню)
  }
...
async function loadMessages(silent = false) {
  ...
  try {
    ...
    console.log('[ChatView] loadMessages - loaded', response.messages?.length, 'messages')
    
    loadedMessageIds.value.clear()
    const normalizedMessages = response.messages.map(m => {
      loadedMessageIds.value.add(m.id)
      return normalizeMessageFields(m)
    })
    
    // Логируем системные сообщения
    const systemMessages = normalizedMessages.filter(m => m.type === 'System')
    console.log('[ChatView] loadMessages - system messages count:', systemMessages.length)
    if (systemMessages.length > 0) {
      console.log('[ChatView] loadMessages - system messages:', systemMessages.map(m => ({
        id: m.id,
        type: m.type,
  ...
  try {
    ...
    if (response.messages.length > 0) {
      const newMessages = response.messages
        .filter(m => !loadedMessageIds.value.has(m.id))
        .map(m => {
          loadedMessageIds.value.add(m.id)
          return normalizeMessageFields(m)
        })
...
// Обработка inline записи голосового
async function onInlineVoiceRecorded({ file, duration }) {
  showInlineVoiceRecorder.value = false
  await onVoiceRecorded({ file, duration })
}
...
async function updateChatFromEdit() {
  try {
    await apiChatUpdateRoute({ feedId: props.feedId }).run(ctx, editChat.value)
    // Обновляем chat реактивно, включая displayTitle
    chat.value = { 
      ...chat.value, 
      ...editChat.value,
      displayTitle: editChat.value.title  // Обновляем displayTitle для шапки
    }
    activeModal.value = null
