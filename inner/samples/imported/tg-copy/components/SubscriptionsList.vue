...
async function openExtendModal(subscription) {
  ...
  if (subscription.plan?.id) {
    try {
      const response = await fetch(`/tg/api/chat-subscription-plans~plans/${subscription.plan.id}/periods`).then(r => r.json())
...
async function confirmExtend() {
  ...
  try {
    const response = await fetch(`/tg/api/chat-subscriptions~${extendingSubscription.value.id}/extend`, {
      method: 'POST',
...
async function loadSubscriptions() {
  loading.value = true
  try {
    const response = await fetch('/tg/api/chat-subscriptions~my').then(r => r.json())
...
async function cancelRenewal(sub) {
  ...
  try {
    await fetch(`/tg/api/chat-subscriptions~${sub.id}/cancel`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({})
    })
