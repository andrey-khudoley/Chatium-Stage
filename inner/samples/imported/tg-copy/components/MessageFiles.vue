  ...
  <div v-if="files && files.length > 0" class="message-files">
    ...
    >
      ...
      <template v-else-if="isVideoNote(file)">
        <div @click.stop class="video-note-wrapper">
          <VideoNote 
            :file="file" 
      ...
      <template v-else-if="isVideo(file)">
        <div class="image-wrapper" @click.stop="openViewer(index)">
          <video
            ...
            class="file-video"
            preload="metadata"
            @loadedmetadata="onVideoLoaded(file.hash)"
            muted
          />
          <!-- Спиннер пока видео не загружено -->
          <div v-if="!videoLoadStates[file.hash]" class="video-loading-placeholder">
...
// Объявляем props ДО использования в функциях
...
// Состояние загрузки видео для превью
const videoLoadStates = ref({})

function onVideoLoaded(hash) {
  if (hash) {
    videoLoadStates.value[hash] = true
  }
}
...
function isVideo(file) {
  ...
  // Проверяем расширение — только видео форматы (не audio!)
  // webm может быть и видео и аудио, проверяем по MIME или другим признакам
  if (/\.(mp4|mov|avi|mkv|flv|wmv)$/i.test(name)) return true
  if (/\.webm$/i.test(name) && !mimeType.startsWith('audio/')) return true
  if (/\.ogg$/i.test(name) && !mimeType.startsWith('audio/')) return true
...
function isAudio(file) {
  ...
  // Расширения аудио (включая .weba и .webm когда это аудио)
  if (/\.(mp3|m4a|ogg|wav|flac|weba|opus)$/i.test(name)) return true
  if (/\.webm$/i.test(name) && mimeType.startsWith('audio/')) return true
...
/* Обертка для видео-кружка для предотвращения всплытия событий */
.video-note-wrapper {
  display: inline-flex;
  align-items: center;
...
.video-loading-placeholder .video-loading-text {
  font-size: 14px;
