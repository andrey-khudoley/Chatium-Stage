  ...
  <div class="modal-overlay" @click="$emit('close')">
    <div class="modal-content user-profile-modal" @click.stop>
      ...
      <div class="profile-body">
        ...
        <!-- Контакты видны только сотрудникам или себе -->
        <div v-if="(user?.phone || user?.email) && canViewContacts" class="profile-section">
          <h3>Контакты</h3>
...
const canViewContacts = computed(() => {
  // Сам пользователь может видеть свои контакты
  if (isCurrentUser.value) return true
  // Сотрудники, админы и владельцы могут видеть контакты других
  const role = props.currentUserRole || ctx.user?.accountRole
  return role === 'Staff' || role === 'Admin' || role === 'Owner' || role === 'Developer'
})
