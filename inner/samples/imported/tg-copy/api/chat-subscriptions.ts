...
export const apiChatSubscriptionGetRoute = app.get('/:feedId/subscription', async (ctx, req) => {
  ...
  // Извлекаем planIds - planId может быть строкой или объектом с .id
  const planIds = planChatLinks.map(link => {
    const id = typeof link.planId === 'string' ? link.planId : link.planId?.id;
    return id;
  }).filter(Boolean);
  ...
  // Ищем подписку на любой из этих тарифов (включая expired для показа информации)
  ...
  // Определяем реальный статус подписки
  const now = new Date();
  let actualStatus = subscription.status;
  
  // Если подписка 'active' или 'pending', но endDate прошёл - считаем expired
  if ((subscription.status === 'active' || subscription.status === 'pending') && subscription.endDate < now) {
    actualStatus = 'expired';
  }

  // Получаем информацию о тарифе
  const planId = typeof subscription.planId === 'string' ? subscription.planId : subscription.planId?.id;
  const plan = planId ? await SubscriptionPlans.findById(ctx, planId) : null;

  // Получаем все чаты из тарифа
  const planChats = plan ? await PlanChats.findAll(ctx, {
  ...
  return {
    ...
    isExpiringSoon: actualStatus === 'active' ? isExpiringSoon(subscription) : false
  };
});
