  ...
  .handle(async (ctx, body) => {
    ...
    try {
      ...
      if (!settings?.tgManagerId) {
        return {
          ok: false,
          result: 'TG Manager ID не настроен. Перейдите в настройки инструмента и выберите бота-менеджера.',
        }
      }
      ...
      if (!settings?.groupOrChannelId) {
        return {
          ok: false,
          result: 'ID группы/канала не настроен. Перейдите в настройки инструмента и укажите ID группы или канала.',
        }
      }
