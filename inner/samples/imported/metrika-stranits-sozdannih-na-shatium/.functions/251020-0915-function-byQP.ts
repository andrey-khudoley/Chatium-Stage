async function main(ctx: app.Ctx) {
  ...
  const engagementResult = await apiGetEngagementStatsRoute.query({
    period: '30'
  }).run(ctx);
  
  // Тест 2: scroll-heatmap для /pu-v2-8
  const heatmapResult = await apiGetScrollHeatmapRoute.query({
    path: '/pu-v2-8'
  }).run(ctx);
  ...
  return {
    ...
    heatmap: {
      ...
      dataRanges: heatmapResult.data?.map(d => ({ range: d.depth_range, users: d.users_count, percent: d.percentage }))
    }
  };
}
