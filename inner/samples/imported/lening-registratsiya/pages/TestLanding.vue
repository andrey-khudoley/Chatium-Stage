    ...
    <div class="max-w-2xl mx-auto">
      ...
      <div v-if="step === 'quiz'" class="card mb-6 text-center">
        <h1 class="text-3xl md:text-4xl font-bold text-dark mb-3">
          Тест скорости мастера 💅
        </h1>
        <p class="text-lg text-gray-600 mb-4">
          Узнайте свой реальный <span class="text-primary font-bold">потенциал заработка!</span> Получите <span class="text-accent font-bold">персональный план развития</span> и <span class="text-primary font-bold">эксклюзивный подарок</span> в конце теста.
        </p>
      ...
      <transition name="fade" mode="out-in">
        <div v-if="step === 'quiz'" :key="'quiz-' + currentQuestion" class="card">
          ...
          <div class="mb-8">
            <div class="flex justify-between items-center mb-4">
              <span class="text-sm font-semibold text-gray-600">Вопрос {{ currentQuestion + 1 }} из {{ questions.length }}</span>
              <span class="text-sm text-gray-500">~{{ Math.ceil((questions.length - currentQuestion) * 12) }} сек</span>
            </div>
              ...
              >
                <div class="flex items-center">
                  <div class="w-6 h-6 rounded-full border-2 mr-4 flex items-center justify-center"
                       :class="selectedAnswer === idx ? 'border-primary bg-primary' : 'border-gray-300'">
        ...
        <div v-if="step === 'results'" :key="'results'" class="card">
          ...
          <div class="bg-gradient-to-br from-pink-50 to-purple-50 rounded-3xl p-8 mb-24 border-2 border-primary shadow-xl">
            <div class="text-center mb-6">
              <span class="inline-block bg-gradient-to-r from-primary to-accent text-white px-6 py-2 rounded-full text-sm font-bold mb-4">🎁 ВАШ ПОДАРОК</span>
              ...
              <p class="text-xl text-gray-800 leading-relaxed font-medium">
                Верхние формы — лучшая техника моделирования для быстрого набора клиентов и роста дохода. 
                Подходит мастерам любого уровня — от новичков до профи!
              </p>
            </div>
            ...
            <div class="bg-white rounded-2xl p-6 mb-5 shadow-md">
              ...
              <ul class="space-y-3 text-base text-gray-800">
                ...
                <li class="flex items-start">
                  <span class="text-primary mr-2 font-bold">•</span>
                  <span><strong class="text-dark">Мастер-класс:</strong> Выкладной френч в скоростной технике</span>
                </li>
              </ul>
            </div>
            ...
            <div class="bg-white rounded-2xl p-6 mb-6 shadow-md">
              ...
              <ul class="space-y-3 text-base text-gray-800">
                ...
                <li class="flex items-start">
                  <span class="text-accent mr-2 font-bold">•</span>
                  <span><strong class="text-dark">Мастер-класс:</strong> Скоростное арочное моделирование на верхние формы + аквариумный дизайн</span>
                </li>
              </ul>
            </div>
          ...
          <!-- Sticky Button -->
          <div class="fixed bottom-0 left-0 right-0 bg-white shadow-2xl border-t-4 border-primary p-4 z-50">
            <div class="max-w-2xl mx-auto">
              <button @click="step = 'registration'" class="btn-primary w-full text-xl py-4 font-bold shadow-lg">
                🎁 Забрать подарок
              </button>
            </div>
          </div>
        ...
        <div v-if="step === 'registration'" :key="'registration'" class="card relative">
          ...
          <h2 class="text-3xl font-bold text-dark mb-4 text-center">
            Регистрация на марафон
          </h2>
          
          <!-- Timer -->
          <div class="bg-gradient-to-r from-primary to-accent text-white rounded-xl p-4 mb-6 text-center">
          ...
          >
            ...
            <!-- UTM fields -->
            <input name="formParams[dealCustomFields][1297809]" type="hidden" data-id="field-input-1297809">
            <input name="formParams[dealCustomFields][1297810]" type="hidden" data-id="field-input-1297810">
            <input name="formParams[dealCustomFields][1297811]" type="hidden" data-id="field-input-1297811">
            <input name="formParams[dealCustomFields][1297812]" type="hidden" data-id="field-input-1297812">
            <input name="formParams[dealCustomFields][1297813]" type="hidden" data-id="field-input-1297813">
            
            <!-- Helper fields -->
            <input type="hidden" id="150339868fa1f6ab38b5" name="__gc__internal__form__helper" class="__gc__internal__form__helper" value="">
            <input type="hidden" id="150339868fa1f6ab38b5ref" name="__gc__internal__form__helper_ref" class="__gc__internal__form__helper_ref" value="">
            <input type="hidden" name="requestTime" value="1761222506">
            <input type="hidden" name="requestSimpleSign" value="8777e6cd84e3d26397b30076dab32039">
...
const result = computed(() => {
  ...
  if (percentage >= 80) {
    level = 'Мастер-профи'
    description = 'Вы работаете очень быстро! У вас отличная техника и организация рабочего процесса.'
    insight = '💎 Ваш потенциал дохода: до 200 000 ₽/мес'
    potential = 'При вашей скорости вы можете масштабировать бизнес: обучать других мастеров или открыть свою студию.'
  } else if (percentage >= 60) {
    level = 'Опытный мастер'
    description = 'У вас хорошая скорость работы, но есть куда расти. С правильными техниками вы можете увеличить доход.'
    insight = '⭐ Ваш потенциал дохода: до 150 000 ₽/мес'
    potential = 'Ускорив работу на 30-40%, вы сможете принимать на 2-3 клиента больше в день - это +40 000₽ к доходу!'
  } else if (percentage >= 40) {
    level = 'Развивающийся мастер'
    description = 'Вы на правильном пути! Освоение техник скоростной работы поможет вам выйти на новый уровень.'
    ...
    description = 'У вас большой потенциал для роста! Правильные техники помогут вам работать в 2-3 раза быстрее.'
    insight = '🚀 Ваш потенциал дохода: до 120 000 ₽/мес'
    potential = 'Освоив скоростные техники, вы сможете работать быстрее и зарабатывать больше уже через месяц!'
  }
...
window.addEventListener('load', function(){
  ...
  let statUrl = "https://emi-courses.ru/stat/counter?ref=" + encodeURIComponent(document.referrer)
