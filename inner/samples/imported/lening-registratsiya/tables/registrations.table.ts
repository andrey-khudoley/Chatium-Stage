...
export const TVfRegistratsiyaRegistrationsYLp = Heap.Table(
  't_vf-registratsiya_registrations_c7M',
  {
    firstName: Heap.Optional(
      Heap.String({ customMeta: { title: 'Имя' }, searchable: { langs: ['ru', 'en'], embeddings: true } }),
    ),
    phone: Heap.Optional(
    ...
    testScore: Heap.Optional(
      Heap.Number({ customMeta: { title: 'Результат теста' }, searchable: { langs: ['ru', 'en'], embeddings: true } }),
    ),
    speedLevel: Heap.Optional(
      Heap.String({ customMeta: { title: 'Уровень скорости' }, searchable: { langs: ['ru', 'en'], embeddings: true } }),
    ),
