...
export const TAnalitikaLendovTAnalitikaLendovPaymentsPayB79 = Heap.Table(
  't_analitika-lendov_t_analitika-lendov_payments_Pay_1d4',
  {
    ...
    date: Heap.Optional(
      Heap.String({ customMeta: { title: 'Дата (для совместимости)' }, searchable: { langs: ['ru', 'en'] } }),
    ),
    amount: Heap.Optional(Heap.Number({ customMeta: { title: 'Сумма оплаты' } })),
    status: Heap.Optional(Heap.String({ customMeta: { title: 'Статус оплаты' }, searchable: { langs: ['ru', 'en'] } })),
...
export type TAnalitikaLendovTAnalitikaLendovPaymentsPayB79Row = typeof TAnalitikaLendovTAnalitikaLendovPaymentsPayB79.T
