...
// @shared-route
export const apiGetEngagementStatsRoute = app.query(s => ({
  ...
  try {
    ...
    const engagementPromises = landings.map(async (landing) => {
      ...
      const workspaceData = await loadWorkspaceData(ctx, landing.workspacePath || '', periodStart, periodEnd)
      ...
      try {
        ...
        const conversionRate = sessionsCount > 0 ? ((registrationsCount / sessionsCount) * 100).toFixed(2) : '0.00'
    ...
    ctx.account.log('Engagement stats loaded', {
      ...
      json: {
        ...
        avgSeconds: data.reduce((sum, item) => sum + item.avg_duration_seconds, 0) / (data.length || 1)
      }
    })
...
// @shared-route
export const apiGetScrollHeatmapRoute = app.query(s => ({
  path: s.string(),
  period: s.string().optional(),
  startDate: s.string().optional(),
  endDate: s.string().optional()
...
// @shared-route
export const apiGetUtmStatsRoute = app.query(s => ({
  period: s.string().optional(),
  startDate: s.string().optional(),
  endDate: s.string().optional(),
  landingPath: s.string().optional()
  ...
  try {
    ...
    const landingDataPromises = landings.map(async (landing) => {
      ...
      const workspaceDataForUtm = await loadWorkspaceData(ctx, landing.workspacePath || '', periodStartUtm, periodEndUtm)
      ...
      for (const row of result.rows) {
        ...
        const conversionRate = row.sessions_count > 0 ? ((conversionsCount / row.sessions_count) * 100).toFixed(2) : '0.00'
        ...
        hierarchy[source].durations.push({ avg_ms: row.avg_duration_ms, min_ms: row.min_duration_ms, max_ms: row.max_duration_ms })
        ...
        hierarchy[source].children[medium].durations.push({ avg_ms: row.avg_duration_ms, min_ms: row.min_duration_ms, max_ms: row.max_duration_ms })
        ...
        hierarchy[source].children[medium].children[campaign].conversions_count += conversionsCount
        hierarchy[source].children[medium].children[campaign].durations.push({ avg_ms: row.avg_duration_ms, min_ms: row.min_duration_ms, max_ms: row.max_duration_ms })
        ...
        hierarchy[source].children[medium].children[campaign].children[term].visits_count += row.visits_count
        hierarchy[source].children[medium].children[campaign].children[term].sessions_count += row.sessions_count
        hierarchy[source].children[medium].children[campaign].children[term].conversions_count += conversionsCount
        hierarchy[source].children[medium].children[campaign].children[term].durations.push({ avg_ms: row.avg_duration_ms, min_ms: row.min_duration_ms, max_ms: row.max_duration_ms })
      ...
      const calculateAggregatedDuration = (durations: any[]) => {
        ...
        const avgMs = validDurations.reduce((sum, d) => sum + (d.avg_ms || 0), 0) / validDurations.length
        const minMs = Math.min(...validDurations.map(d => d.min_ms || Infinity).filter(v => v !== Infinity && v > 0))
      ...
      for (const source in hierarchy) {
        ...
        sourceNode.conversion_rate = sourceNode.sessions_count > 0 ? parseFloat(((sourceNode.conversions_count / sourceNode.sessions_count) * 100).toFixed(2)) : 0
        ...
        for (const medium in sourceNode.children) {
          ...
          mediumNode.conversion_rate = mediumNode.sessions_count > 0 ? parseFloat(((mediumNode.conversions_count / mediumNode.sessions_count) * 100).toFixed(2)) : 0
          ...
          for (const campaign in mediumNode.children) {
            ...
            campaignNode.conversion_rate = campaignNode.sessions_count > 0 ? parseFloat(((campaignNode.conversions_count / campaignNode.sessions_count) * 100).toFixed(2)) : 0
            ...
            for (const term in campaignNode.children) {
              ...
              termNode.conversion_rate = termNode.sessions_count > 0 ? parseFloat(((termNode.conversions_count / termNode.sessions_count) * 100).toFixed(2)) : 0
...
// @shared-route
export const apiGetDateStatsRoute = app.query(s => ({
  period: s.string().optional(),
  startDate: s.string().optional(),
  endDate: s.string().optional(),
  landingPath: s.string().optional()
  ...
  try {
    ...
    const dateDataPromises = landings.map(async (landing) => {
      // Загружаем регистрации из воркспейса лендинга
      const workspaceDataForDate = await loadWorkspaceData(ctx, landing.workspacePath || '', periodStart, periodEnd)
    ...
    for (const [dateKey, data] of dateGroups) {
      const { year, month, day, visits_count, sessions_count, conversions_count, durations } = data
    ...
    for (const year in hierarchy) {
      ...
      yearNode.conversion_rate = yearNode.sessions_count > 0 ? ((yearNode.conversions_count / yearNode.sessions_count) * 100) : 0
      ...
      for (const monthKey in yearNode.children) {
        ...
        monthNode.conversion_rate = monthNode.sessions_count > 0 ? ((monthNode.conversions_count / monthNode.sessions_count) * 100) : 0
...
// @shared-route
export const apiGetRegistrationsByDayRoute = app.query(s => ({
  period: s.string().optional(),
  startDate: s.string().optional(),
  endDate: s.string().optional(),
  landingPath: s.string().optional()
  ...
  try {
    ...
    for (const landing of landings) {
      // Загружаем регистрации из воркспейса лендинга
      const workspaceDataForReg = await loadWorkspaceData(ctx, landing.workspacePath || '', periodStart, periodEnd)
...
// @shared-route
export const apiGetDemographicsRoute = app.query(s => ({
  period: s.string().optional(),
  startDate: s.string().optional(),
  endDate: s.string().optional(),
  landingPath: s.string().optional()
...
// @shared-route
export const apiGetDevicesRoute = app.query(s => ({
  period: s.string().optional(),
  startDate: s.string().optional(),
  endDate: s.string().optional(),
  landingPath: s.string().optional()
...
// @shared-route
export const apiGetSessionsByDayRoute = app.query(s => ({
  period: s.string().optional(),
  startDate: s.string().optional(),
  endDate: s.string().optional(),
  landingPath: s.string().optional()
