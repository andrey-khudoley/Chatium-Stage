  ...
  <div>
    ...
    <div v-if="step === 'email'">
      ...
      <button
        ...
        :disabled="loading || !isValidEmail(email) || (usePassword && !password)"
...
const sendCode = async () => {
  if (!isValidEmail(email.value)) {
    error.value = ctx.t('Enter valid email address')
    return
  }
  ...
  try {
    ...
    error.value = ctx.t('Error sending email')
  } finally {
    loading.value = false
  }
}
...
const confirmCode = async () => {
  if (verificationCode.value.length < 6) {
    error.value = ctx.t('Enter 6-digit code')
    return
  }
