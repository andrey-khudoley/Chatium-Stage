<template>
  <div class="min-h-screen text-white overflow-x-hidden" style="background: linear-gradient(180deg, #0a0a0f 0%, #111118 100%);">
    <!-- Animated Background -->
    <div class="fixed inset-0 overflow-hidden pointer-events-none">
      <div class="absolute top-0 -left-40 w-96 h-96 bg-purple-600/20 rounded-full blur-3xl animate-blob"></div>
      <div class="absolute top-1/3 -right-40 w-96 h-96 bg-cyan-500/20 rounded-full blur-3xl animate-blob animation-delay-2000"></div>
      <div class="absolute -bottom-40 left-1/3 w-96 h-96 bg-pink-500/15 rounded-full blur-3xl animate-blob animation-delay-4000"></div>
    </div>
    ...
    <div class="relative z-10 max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
            ...
            <div class="grid lg:grid-cols-2 gap-8">
              ...
              <div class="space-y-6">
                ...
                <div class="space-y-2">
                  ...
                  <select v-model="imageForm.model" class="w-full px-4 py-3 rounded-xl bg-white/5 border border-white/10 text-white focus:outline-none focus:ring-2 focus:ring-purple-500/50 focus:border-purple-500/50 transition-all duration-200 cursor-pointer">
                  ...
                  <div class="grid grid-cols-2 gap-3">
                    <button
                      ...
                      :class="[
                        ...
                        imageForm.mode === 'text-to-image' ? 'bg-purple-500/20 border-purple-500/50 text-purple-300' : ''
                      ]"
                    >
                    ...
                    <button
                      ...
                      :class="[
                        'flex flex-col items-center justify-center py-4 px-3 rounded-xl bg-white/5 border border-white/10 text-gray-400 hover:bg-white/10 hover:text-white transition-all duration-200',
                        imageForm.mode === 'image-to-image' ? 'bg-purple-500/20 border-purple-500/50 text-purple-300' : ''
                      ]"
                    >
                      <svg class="w-6 h-6 mb-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
                ...
                <button
                  @click="generateImage"
                  :disabled="isGenerating || !imageForm.prompt || (imageForm.mode === 'image-to-image' && !imageForm.uploadedImageHash)"
              ...
              <div v-if="imageForm.mode === 'image-to-image'" class="space-y-4">
                ...
                <div v-if="uploadedImages.length > 0">
                  ...
                  <div class="grid grid-cols-4 gap-2">
                    ...
                    >
                      <img :src="`https://fs.cdn-chatium.io/get/${img.uploadedImageHash}`" class="w-full h-full object-cover" />
                      <div v-if="imageForm.uploadedImageHash === img.uploadedImageHash" class="absolute inset-0 bg-purple-500/20 flex items-center justify-center">
            ...
            <div class="grid lg:grid-cols-2 gap-8">
              ...
              <div class="space-y-6">
                ...
                <div class="space-y-2">
                  ...
                  <select v-model="videoForm.model" class="w-full px-4 py-3 rounded-xl bg-white/5 border border-white/10 text-white focus:outline-none focus:ring-2 focus:ring-cyan-500/50 focus:border-cyan-500/50 transition-all duration-200 cursor-pointer">
                ...
                <div class="space-y-2">
                  ...
                  <div class="grid grid-cols-2 gap-3">
                    <button
                      ...
                      :class="[
                        'flex flex-col items-center justify-center py-4 px-3 rounded-xl bg-white/5 border border-white/10 text-gray-400 hover:bg-white/10 hover:text-white transition-all duration-200',
                        videoForm.mode === 'text-to-video' ? 'bg-cyan-500/20 border-cyan-500/50 text-cyan-300' : ''
                      ]"
                    >
                      <svg class="w-6 h-6 mb-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                    ...
                    <button
                      ...
                      :class="[
                        'flex flex-col items-center justify-center py-4 px-3 rounded-xl bg-white/5 border border-white/10 text-gray-400 hover:bg-white/10 hover:text-white transition-all duration-200',
                        videoForm.mode === 'image-to-video' ? 'bg-cyan-500/20 border-cyan-500/50 text-cyan-300' : ''
                      ]"
                    >
                      <svg class="w-6 h-6 mb-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
                ...
                <button
                  @click="generateVideo"
                  :disabled="isGenerating || !videoForm.prompt || (videoForm.mode === 'image-to-video' && !videoForm.uploadedImageHash)"
              ...
              <div v-if="videoForm.mode === 'image-to-video'" class="space-y-4">
                ...
                <div v-if="uploadedImages.length > 0">
                  ...
                  <div class="grid grid-cols-4 gap-2">
                    ...
                    >
                      <img :src="`https://fs.cdn-chatium.io/get/${img.uploadedImageHash}`" class="w-full h-full object-cover" />
                      <div v-if="videoForm.uploadedImageHash === img.uploadedImageHash" class="absolute inset-0 bg-cyan-500/20 flex items-center justify-center">
                        <svg class="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
                        </svg>
                      </div>
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            >
              ...
              <div class="flex items-center justify-between mb-3">
                <span :class="getTypeBadgeClass(gen.type)" class="px-3 py-1 rounded-full text-xs font-medium">
                  {{ getTypeLabel(gen.type) }}
                </span>
                <span :class="getStatusBadgeClass(gen.status)" class="px-2 py-1 rounded-full text-xs font-medium">
              ...
              <div class="relative aspect-square rounded-xl overflow-hidden bg-white/5 mb-3">
                <div v-if="gen.status === 'processing'" class="absolute inset-0 flex items-center justify-center">
                ...
                <div v-else-if="gen.status === 'failed'" class="absolute inset-0 flex items-center justify-center">
              ...
              <!-- Prompt -->
              <p v-if="gen.prompt" class="text-sm text-gray-400 line-clamp-2 mb-3">{{ gen.prompt }}</p>
              <p v-if="gen.error" class="text-sm text-red-400 line-clamp-2 mb-3">{{ gen.error }}</p>

              <!-- Download Button -->
              <button
                v-if="gen.generatedImageHash"
                @click="downloadFile(`https://fs.cdn-chatium.io/get/${gen.generatedImageHash}`, 'image.png')"
      ...
      <div v-else class="glass-card rounded-3xl p-1">
        <div class="rounded-3xl p-12 text-center" style="background: rgba(17, 17, 24, 0.8);">
          <div class="w-20 h-20 mx-auto mb-6 rounded-2xl bg-gradient-to-br from-purple-500/20 to-cyan-500/20 flex items-center justify-center">
...
import { apiGenerateImageRoute, apiGenerateVideoRoute, apiGetGenerationsRoute, apiUploadImageRoute } from '../api/generations'
...
const uploadedImages = computed(() => {
  return generations.value.filter(g => g.type === 'upload' && g.uploadedImageHash && g.status === 'completed')
})
