...
export const TKalendarZapisiAdminUsersBth = Heap.Table(
  't_kalendar-zapisi_adminUsers_ydI',
  {
    ...
    lastName: Heap.Optional(
      Heap.String({ customMeta: { title: 'Фамилия' }, searchable: { langs: ['ru', 'en'], embeddings: true } }),
    ),
    role: Heap.Optional(
      Heap.String({ customMeta: { title: 'Роль' }, searchable: { langs: ['ru', 'en'], embeddings: true } }),
    ),
    isActive: Heap.Optional(
      Heap.Boolean({ customMeta: { title: 'Активен' }, searchable: { langs: ['ru', 'en'], embeddings: true } }),
    ),
    isOwner: Heap.Optional(
      Heap.Boolean({ customMeta: { title: 'Владелец' }, searchable: { langs: ['ru', 'en'], embeddings: true } }),
    ),
  },
