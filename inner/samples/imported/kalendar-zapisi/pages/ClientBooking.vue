  ...
  <div class="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 py-8 px-4">
    <div class="max-w-6xl mx-auto">
      <div class="bg-white rounded-lg shadow-lg p-8">
        <h1 class="text-3xl font-bold text-gray-800 mb-2">{{ calendarSettings?.title || 'Запись на встречу' }}</h1>
        <p class="text-gray-600 mb-6">Выберите удобное для вас время</p>

        <div v-if="calendarSettings && !calendarSettings.isActive" class="p-4 bg-red-50 border border-red-200 rounded-lg">
          <p class="text-red-800 font-medium">Запись на встречу закрыта</p>
          <p class="text-red-700 text-sm">В данный момент записи на встречу недоступны. Пожалуйста, свяжитесь с нами позже.</p>
        </div>
            ...
            <div class="overflow-x-auto">
              <div class="flex gap-1 min-w-max">
                ...
                <div class="flex-shrink-0">
                  <div class="h-12 flex items-end px-2 font-semibold text-gray-700 text-sm">Время</div>
                  <div v-for="time in allTimes" :key="time" class="h-12 flex items-center px-2 border-b border-gray-200 text-xs font-medium text-gray-600">
                    {{ time }}
                  </div>
                </div>
                ...
                <div v-for="date in visibleDates" :key="date" class="flex-shrink-0">
                  <div class="w-32">
                    <!-- Day Header -->
                    <div class="h-12 flex flex-col items-center justify-center px-2 font-semibold text-gray-800 text-xs bg-gray-50 border-b-2 border-gray-300">
                    ...
                    <div v-if="schedule[date]">
                      <button
                        ...
                        :class="{
                          'bg-green-100 text-green-800 hover:bg-green-200 cursor-pointer': slot.available,
                          'bg-red-100 text-red-800 cursor-not-allowed': !slot.available && slot.booked,
                          'bg-gray-100 text-gray-400 cursor-not-allowed': !slot.available && !slot.booked
                        }"
                      >
                        {{ slot.available ? '✓ Свободно' : (slot.booked ? '✗ Занято' : 'Недоступно') }}
                      </button>
                    </div>
                    <div v-else class="p-4 text-gray-500 text-xs text-center">
                      Выходной
                    </div>
                  </div>
                </div>
...
import { apiCalendarSettingsRoute, apiScheduleRoute, apiCreateAppointmentRoute } from '../api/appointments';
