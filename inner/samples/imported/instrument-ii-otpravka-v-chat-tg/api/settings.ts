...
import { getChannels, runTelegramApi, getTelegramGroups, createOrUpdateChannelBySecret, ChannelSource } from '@sender/sdk'
...
export const apiGetBotsRoute = app.get('/bots', async (ctx, req) => {
  try {
    ...
    const telegramBots = allChannels.filter(channel => 
      channel.source === 'Telegram' && channel.active
    )
    return {
      success: true,
...
// @shared-route
export const apiCreateAgentRoute = app.body(s => ({
  title: s.string(),
  prompt: s.string()
...
export const apiGetSettingsRoute = app.get('/get-settings', async (ctx, req) => {
  try {
    ...
    const settings = {
      tgBotId: config.tgBotId || '',
      tgChannelId: config.tgChannelId || '',
      messageWrapper: config.messageWrapper || '🤖 <b>Сообщение от ИИ агента</b>\n\n{TEXT}\n\n<i>Отправлено автоматически</i>',
      agentId: config.agentId || ''
    }
...
// @shared-route
export const apiTestToolRoute = app.body(s => ({
  tgBotId: s.string(),
  tgChannelId: s.string(),
  messageWrapper: s.string(),
  ...
  try {
    ...
    const messageText = messageWrapper.replace(/{TEXT}/g, 'Это тестовое сообщение от инструмента отправки в Telegram')
...
// @shared-route
export const apiConnectTelegramRoute = app.body(s => ({
  token: s.string()
