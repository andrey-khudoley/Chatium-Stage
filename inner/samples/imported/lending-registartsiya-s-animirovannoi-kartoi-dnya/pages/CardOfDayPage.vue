    ...
    <div class="relative z-10">
      ...
      <header class="pt-8 pb-6 text-center px-4" v-if="!selectedCard">
        <div class="animate-fade-in max-w-4xl mx-auto">
          <!-- Декоративная рамка -->
          <div class="relative">
            <div class="absolute top-0 left-1/2 transform -translate-x-1/2 w-32 h-px bg-gradient-to-r from-transparent via-amber-400/60 to-transparent"></div>
            
            <div class="mt-8 mb-12">
              <h1 class="text-6xl md:text-8xl font-serif font-bold mb-6 text-amber-50 tracking-tight" style="text-shadow: 0 0 40px rgba(251, 191, 36, 0.3), 0 2px 4px rgba(0,0,0,0.5);">
                Карта Дня Мастера
              </h1>
              ...
              <p class="text-xl md:text-2xl text-amber-100/90 font-light tracking-wide leading-relaxed">
                Узнай, что звезды приготовили<br class="hidden md:block" /> для твоего бизнеса сегодня
              </p>
            </div>
            
            <div class="absolute bottom-0 left-1/2 transform -translate-x-1/2 w-32 h-px bg-gradient-to-r from-transparent via-amber-400/60 to-transparent"></div>
          </div>
          
          <!-- Три ключевых посыла -->
          <div class="max-w-2xl mx-auto mt-16 space-y-6">
            <div class="flex items-start gap-4 text-left bg-slate-700 rounded-2xl p-6 border border-amber-500/10 transition-all">
      ...
      <div v-if="!selectedCard && !isShuffling" class="container mx-auto px-4 py-8">
        <div class="text-center mb-12 max-w-2xl mx-auto">
          ...
          <div class="relative py-12">
            <div class="absolute top-0 left-1/2 transform -translate-x-1/2 w-48 h-px bg-gradient-to-r from-transparent via-amber-400/50 to-transparent"></div>
            ...
            <div class="absolute bottom-0 left-1/2 transform -translate-x-1/2 w-48 h-px bg-gradient-to-r from-transparent via-amber-400/50 to-transparent"></div>
          </div>
        </div>
      ...
      <div v-if="isShuffling" class="container mx-auto px-4 py-12">
        <div class="flex justify-center items-center min-h-[400px]">
          <div class="card-shuffle-animation">
            <div v-for="i in 7" :key="i" class="shuffle-card" :style="{ animationDelay: `${i * 0.1}s` }">
              <div class="card-back">
                <div class="card-pattern"></div>
              </div>
            </div>
      ...
      <div v-if="selectedCard" class="container mx-auto px-4 py-12 max-w-4xl">
        <div class="card-reveal-animation">
          <div 
            class="card-front mx-auto mb-8 rounded-3xl overflow-hidden shadow-2xl"
          >
            <div class="relative">
              <img :src="selectedCard.imageUrl" :alt="selectedCard.title" class="w-full h-auto object-cover" />
              <div class="absolute inset-0 bg-gradient-to-t from-black/80 via-black/40 to-transparent flex flex-col justify-end p-8">
                ...
                <h2 class="text-4xl font-bold mb-2 text-white text-center">{{ selectedCard.title }}</h2>
          ...
          <div class="bg-slate-700 rounded-3xl p-8 mb-6 border border-amber-500/20 shadow-2xl">
            ...
            <div class="bg-gradient-to-r from-amber-500/20 to-rose-500/20 rounded-2xl p-6 border border-amber-400/40 shadow-xl">
          ...
          <!-- Призыв к действию -->
          <div class="bg-gradient-to-r from-amber-400 via-rose-500 to-purple-600 rounded-3xl p-1 mb-8 shadow-2xl">
            <div class="bg-gradient-to-br from-slate-950 to-indigo-950 rounded-3xl p-8">
              <div class="text-center mb-6">
                <div class="inline-block bg-gradient-to-r from-rose-500 to-purple-600 px-4 py-2 rounded-full text-sm font-semibold mb-3 shadow-lg">
                  📅 11, 12, 13 ноября в 18:00 МСК
                </div>
                <h3 class="text-3xl md:text-4xl font-bold mb-3 text-amber-100">Как стать богатым ТОП-мастером в 2026 году</h3>
                <p class="text-lg text-amber-100/90 mb-2">
                  Это не просто марафон — это 3 дня интенсивной практики с мастерами,<br class="hidden md:block"/> которые сами выросли до ТОП-уровня
                </p>
              ...
              <div class="grid md:grid-cols-2 gap-4 mb-6">
                <div class="bg-slate-700 rounded-2xl p-4 border border-amber-500/20">
                  ...
                  <img src="https://fs.chatium.io/get/image_gc_c3BS1PR50c.1080x607.jpeg" alt="Мастер-классы практикума" class="w-full rounded-xl mb-4" />
                  <h4 class="font-semibold mb-3 text-amber-300">⭐ 4 практических мастер-класса:</h4>
                ...
                <div class="bg-slate-700 rounded-2xl p-4 border border-amber-500/20">
                  ...
                  <!-- Баннер розыгрыша -->
                  <img src="https://fs.chatium.io/get/image_gc_8THPoWIakn.1080x607.jpeg" alt="Розыгрыш призов" class="w-full rounded-xl mb-4" />
                  <ul class="space-y-2 text-sm text-amber-100/90">
              ...
              <!-- Для кого -->
              <div class="bg-gradient-to-r from-amber-500/20 to-rose-500/20 rounded-2xl p-6 border border-amber-400/40 mb-6 shadow-xl">
                <h4 class="text-xl font-semibold mb-3 text-center text-rose-300">💎 Обязательно приходи, если:</h4>
          ...
          <div id="registration-form" v-if="!isRegistered" class="bg-slate-700 rounded-3xl py-8 px-0 border border-amber-500/20 shadow-2xl">
            <iframe 
              :src="iframeUrl" 
          ...
          <div v-else class="bg-gradient-to-r from-emerald-500/20 to-teal-500/20 rounded-3xl p-8 border border-emerald-400/40 shadow-2xl">
...
.card-back {
  ...
  border-radius: 20px;
  box-shadow: 0 20px 60px rgba(168, 85, 247, 0.5), 0 0 30px rgba(251, 191, 36, 0.3);
...
@keyframes shuffle {
  ...
  25% { transform: translateX(-200px) translateY(-100px) rotate(-20deg); opacity: 0.7; }
  50% { transform: translateX(200px) translateY(-150px) rotate(20deg); opacity: 0.7; }
  75% { transform: translateX(-100px) translateY(-80px) rotate(-10deg); opacity: 0.7; }
}
