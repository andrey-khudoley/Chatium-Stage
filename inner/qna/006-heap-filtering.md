...
// По массиву ID
where: { assignedTo: [userId1, userId2] }
```
...
```typescript
where: { status: ['active', 'pending', 'archived'] }
```
...
const filtered = items.filter(item => 
  !['draft', 'deleted'].includes(item.status)
);
```
...
const items = await Items.findAll(ctx, { limit: 1000 });
const filtered = items.filter(item => 
  item.price > 100 && item.category === catId
);
