...
declare type CustomProvidableIdentityType =
  | 'Phone'
  | 'Email'
  | 'TelegramId'
  | 'ParentAccountUser'
...
declare type GenericLinkJson = [type: HeapObjectType, id: HeapId]
...
declare class AccountHelperClass {
  ...
  main_mobile_app_id: string | null
  ...
  cluster_move_state: ClusterMoveState | null
...
interface ClusterMoveState {
  role: 'Source' | 'Destination'
  otherCluster: string
  status: ClusterMoveStatus
  startTs?: number
  switchTs?: number
  cleanUpTs?: number
}
declare type ClusterMoveStatus =
  /** everything continues to work on the source cluster, data is moving to destination */
  | 'DbCopy'
  /**
   * code changes are frozen on source, but requests are still handled on source
   */
  | 'CodeFreeze'
  /**
   * new requests are proxied to destination, domains are switching over
   * currently running requests and jobs are finishing on source
   * new jobs are scheduled but not running anywhere
   */
  | 'Switch'
  /**
   * source jobs are finished, all scheduled jobs are reanimated on destination
   * write operations are disabled on the source
   * data sync is stopped
   * only on source cluster, on destination state is cleared on this stage
   */
  | 'CleanUp'
...
/** [ [Lang, [ModuleHash, accountId][]][], accountId -> keyLang, accountId -> CgsRepoVersion ] */
export declare type UgcLangBundlesLoadInfo = [
  Array<readonly [Lang, Array<[string, number]>]>,
  PerAccountKeyLangs,
  LoadedLangAccounts,
]
export declare type LoadedLangAccountVersions = Record<number, CgsRepoVersion[]>
export declare type LoadedLangAccounts = Record<number, CgsRepoVersion>
...
export interface UgcUgcFileListItem {
  ...
  buildError?: string
  buildWarnings?: string[] | null | undefined
}
...
export interface UgcChildAccount extends UgcAccount {
  ownerUserIds: HeapId[]
}
